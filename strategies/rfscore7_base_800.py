...
def choose_stocks(watch_date):
    ...
    df = df.replace([np.inf, -np.inf], np.nan).dropna(subset=["RFScore"])
    df = df.sort_values(
        ["RFScore", "ROA", "OCFOA", "DELTA_MARGIN", "DELTA_TURN"],
        ascending=[False, False, False, False, False],
    )
    return df.index.tolist()[: g.hold_num]
...
def rebalance(context):
    ...
    target_stocks = filter_buyable(context, choose_stocks(watch_date))
