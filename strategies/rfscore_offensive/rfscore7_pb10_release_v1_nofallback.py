...
def initialize(context):
    ...
    g.release_name = "RFScore7 PB10 Release V1 NoFallback"
    g.ipo_days = 180
    g.primary_pb_group = [1, 2, 3]
    g.max_pe_ratio = 100
    g.min_roa = 0.5
...
def choose_stocks(watch_date, target_hold_num):
    ...
    primary = sort_candidates(
        df[(df["RFScore"] == 7) & (df["pb_group"].isin(g.primary_pb_group))].copy()
    )
...
def rebalance(context):
    ...
    current_data = get_current_data()
    target_value = context.portfolio.total_value / float(target_hold_num) if target_hold_num > 0 else 0
