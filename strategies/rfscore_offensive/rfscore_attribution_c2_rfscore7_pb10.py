...
def initialize(context):
    ...
    g.release_name = "C2_RFScore7_PB10"
    g.ipo_days = 180
...
def calc_rfscore_table(stocks, watch_date):
    ...
    val = get_valuation(
        stocks, end_date=watch_date, fields=["pb_ratio"], count=1
    )
    val = val.drop_duplicates("code").set_index("code")[["pb_ratio"]]
...
def rebalance(context):
    ...
    candidates = df[(df["RFScore"] == 7) & (df["pb_group"] <= g.primary_pb_group)].copy()
    candidates = sort_candidates(candidates)
    target_stocks = candidates.index.tolist()[:g.hold_num]
