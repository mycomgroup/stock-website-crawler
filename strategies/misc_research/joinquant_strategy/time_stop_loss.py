...
def initialize(context):
    ...
    run_daily(buy, "09:35")
    run_daily(check_time_stop, "10:30")  # 10:30时间止损
...
def buy(context):
    ...
    try:
        st_df = get_extras(
            "is_st", all_stocks, start_date=prev, end_date=prev, df=True
        ).T
        st_df.columns = ["is_st"]
    ...
    if selected:
        ...
        for s in selected[:3]:
            order_value(s, cash)
            g.trades += 1
            g.buy_times[s] = context.current_dt


def check_time_stop(context):
    """10:30时间止损：未盈利则卖出"""
    cd = get_current_data()
    for s in list(context.portfolio.positions):
        pos = context.portfolio.positions[s]
        if pos.closeable_amount > 0 and s in g.buy_times:
            buy_time = g.buy_times[s]
            current_time = context.current_dt
            # 买入时间到当前超过30分钟且未盈利
            if (current_time - buy_time).seconds > 1800:
                if s in cd:
