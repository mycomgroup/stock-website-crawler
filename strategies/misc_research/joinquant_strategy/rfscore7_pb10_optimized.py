...
def initialize(context):
    ...
    g.release_name = "RFScore7 PB10 Optimized V2"
    g.ipo_days = 180
...
def rebalance(context):
    ...
    if target_hold_num <= 0:
        ...
        if target_stocks:
            ...
            max_ratio, industry_summary = summarize_industry_ratio(
                target_stocks, industry_map
            )
    ...
    log.info(
        "release=%s watch_date=%s breadth=%.3f trend_on=%s target_hold_num=%s actual_hold_num=%s primary_count=%s secondary_count=%s cash_ratio=%.2f stop_loss=%s"
        % (
            g.release_name,
            str(watch_date),
