...
def before_trading_start(context):
    ...
    breadth_pct = calculate_breadth(date)
    zt_count = calculate_sentiment(date)
    ...
    log.info(
        "状态路由器: {} | 广度: {:.1f}% | 涨停: {} | 仓位: {}%".format(
            state, breadth_pct, zt_count, position
        )
    )


def calculate_breadth(date):
...
def handle_data(context, data):
    ...
    if abs(diff) > context.portfolio.total_value * 0.05:
