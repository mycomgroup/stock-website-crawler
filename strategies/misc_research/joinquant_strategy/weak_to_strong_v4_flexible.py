...
def buy(context):
    ...
    for s in g.target_list:
        try:
            ...
            # 弹性版：成交额>5亿
            if avg_price_increase < 0.07 or prev_day_data["money"][0] < 5e8:
                continue
            ...
            # 弹性版：市值>20亿
            if valuation_data.empty or valuation_data["market_cap"][0] < 20:
                continue
            ...
            if (
                len(volume_data) < 2
                or volume_data["volume"][-1] <= max(volume_data["volume"][:-1]) * 0.8
            ):
                continue
            ...
            # 弹性版：高开幅度2%~4%
            if open_ratio <= 1.02 or open_ratio >= 1.04:
                continue
