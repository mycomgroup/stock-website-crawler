...
def filter_by_small_eps(stocks, watch_date):
    q = query(
        valuation.code,
    ...
    df = df[df["eps"] > 0]
    df = df.sort_values("circulating_market_cap", ascending=True)
...
def choose_stocks(watch_date, hold_num):
    ...
    complex_factor_list = df.index.tolist()
    small_eps_stocks = filter_by_small_eps(complex_factor_list, watch_date)

    df = df.loc[df.index.isin(small_eps_stocks)]
