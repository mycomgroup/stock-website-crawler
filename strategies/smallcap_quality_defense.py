...
def select_stocks(watch_date, hold_num):
    ...
    df = df.dropna(subset=["debt_ratio", "cash_quality", "roe"])

    df = df[(df["debt_ratio"] < 0.6) & (df["cash_quality"] > 0.8)]

    if len(df) == 0:
        return []

    df = df.sort_values(["roe", "cash_quality"], ascending=[False, False])
...
def rebalance(context):
    ...
    log.info(f"Quality SmallCap Defense: {len(stocks)} stocks")
