# 致敬聚宽_机器学习多因子50只持仓 - RiceQuant版本
# 逻辑：用多因子z-score评分代替机器学习，50只持仓，月度调仓
...
def handle_bar(context, bar_dict):
    ...
    roa_z = zscore(np.array([df.loc[s, 'roa'] if s in df.index else 0 for s in valid]))
    ...
    scores = {valid[i]: mom_z[i] * 0.2 + roe_z[i] * 0.25 + roa_z[i] * 0.2 + pb_z[i] * 0.15 + growth_z[i] * 0.2
