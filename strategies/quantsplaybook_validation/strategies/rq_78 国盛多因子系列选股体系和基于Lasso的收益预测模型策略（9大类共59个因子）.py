# 国盛多因子选股体系 - RiceQuant版本
# 逻辑：9大类因子综合评分（用RQ可用因子近似），月度调仓

import numpy as np

def init(context):
    context.stock_num = 30
    context.month = -1
    # set_benchmark removed (not needed in RQ)
    # set_option removed (not needed in RQ)
def handle_bar(context, bar_dict):
    current_month = context.now.month
    if current_month == context.month:
        return
    context.month = current_month

    stocks = index_components('000905.XSHG')
    if not stocks:
        return

    try:
        prev_date = context.now.date()
        factor_df = get_factor(
            stocks,
            ['pb_ratio', 'roe'],
        )
        if factor_df is None or len(factor_df) == 0:
            return
        df = factor_df.groupby(level=0).last().dropna()
        if df is None or len(df) == 0:
            return
        df = df[df['pb_ratio'] > 0]
        df = df[df['roe'] > 0.00]
        candidates = df.index.tolist()
    except Exception:
        return
    def zscore(arr):
        s = np.std(arr)
        return (arr - np.mean(arr)) / s if s > 0 else np.zeros_like(arr)

    momentum = {}
    for stock in df.index:
        try:
            closes = history_bars(stock, 61, '1d', 'close')
            if closes is not None and len(closes) >= 61:
                momentum[stock] = closes[-1] / closes[0] - 1
        except Exception:
            pass

    valid = [s for s in df.index if s in momentum]
    if len(valid) < 5:
        return

    mom_z = zscore(np.array([momentum[s] for s in valid]))
    roe_z = zscore(np.array([df.loc[s, 'roe'] for s in valid]))
    pb_z = -zscore(np.array([df.loc[s, 'pb_ratio'] for s in valid]))
    growth_z = zscore(np.array([df.loc[s, 'inc_net_profit_year_on_year'] for s in valid]))
    gpm_z = zscore(np.array([df.loc[s, 'gross_profit_margin'] for s in valid]))

    scores = {valid[i]: mom_z[i]*0.2 + roe_z[i]*0.25 + pb_z[i]*0.15 + growth_z[i]*0.2 + gpm_z[i]*0.2
              for i in range(len(valid))}

    sorted_stocks = sorted(scores, key=scores.get, reverse=True)
    target = [s for s in sorted_stocks if (bar_dict[s] if s in bar_dict else None) and bar_dict[s].is_trading][:context.stock_num]
    if not target:
        return

    for stock in list(context.portfolio.positions.keys()):
        if stock not in target:
            order_target_value(stock, 0)
    weight = 1.0 / len(target)
    for stock in target:
        order_target_value(stock, context.portfolio.total_value * weight * 0.95)
