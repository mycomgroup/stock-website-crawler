...
# 并行爬虫执行脚本 - 排除 hao123 配置
# 遍历 config 下所有任务（跳过 hao123），每个任务跑 10 轮，每次并行跑 5 个任务
...
LOG_DIR="$WORK_DIR/logs-nohao123"

# ==================== 初始化 ====================
echo "========================================"
echo "并行爬虫任务启动器 (排除 hao123)"
echo "========================================"
echo "工作目录: $WORK_DIR"
...
# 获取所有配置文件（排除 hao123 开头的和 example.json）
CONFIG_FILES=()
while IFS= read -r -d '' file; do
    filename=$(basename "$file")
    # 排除 example.json 和 hao123 开头的文件
    if [ "$filename" != "example.json" ] && [[ ! "$filename" == hao123-* ]]; then
        CONFIG_FILES+=("$file")
    fi
