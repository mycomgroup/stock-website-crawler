...
def _load_champion_metrics(base: Path, state: dict, obj_cfg: dict):
    """优先从 history 重建 champion，避免旧 state 丢失字段。"""
    ...
    if champion_metrics_raw:
        metrics = ParsedMetrics(
            ...
            total_return=champion_metrics_raw.get("total_return", 0.0),
            ...
            alpha=champion_metrics_raw.get("alpha", 0.0),
            beta=champion_metrics_raw.get("beta", 0.0),
        )
        champion_score = calculate_score(metrics, obj_cfg.get("weights"))
        return champion_score, metrics
...
def main():
    ...
    parse_input = _build_parse_input(
        fetch=fetch,
        backtest_id=backtest_id,
        fallback_status=wait_result.get("status", "normal_exit"),
    )
