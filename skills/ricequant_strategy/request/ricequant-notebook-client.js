...
export class RiceQuantNotebookClient {
  constructor(options = {}) {
    ...
    this.xsrfToken = stripWrappedQuotes(
      options.xsrfToken ||
      this.cookieJar.find(item => item.name === '_xsrf' && hostMatchesDomain('www.ricequant.com', item.domain))?.value
    );
    ...
    if (!this.xsrfToken) {
      ...
      if (sessionCookie) {
        this.xsrfToken = sessionCookie.value;
      }
    }
  }
  ...
  async executeCode(options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      socket.addEventListener('error', () => {
        fail(new Error(`WebSocket 执行失败：${wsUrl}`));
      });

      socket.addEventListener('close', () => {
        if (!settled) {
          fail(new Error('WebSocket 在执行完成前关闭'));
        }
      });
    });
  }
