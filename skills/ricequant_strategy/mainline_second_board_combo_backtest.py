...
# 策略参数
PARAMS = {
    "mainline_weight": 0.5,  # 主线仓位
    "second_board_weight": 0.5,  # 二板仓位
    "scheme": "C",  # 组合方案：A-主线优先，B-二板优先，C-并行独立，D-收益优先
}
...
def init(context):
    ...
    # 设置回测参数
    context.scheme = PARAMS["scheme"]
    context.mainline_weight = PARAMS["mainline_weight"]
    context.second_board_weight = PARAMS["second_board_weight"]
...
def get_mainline_signals(context, bar_dict):
    ...
    try:
        # 获取昨日涨停股票
        yesterday = history_bars("000001.XSHG", 2, "1d", "close")[-2]
        ...
        for stock in check_stocks:
            try:
                ...
                # 今日开盘价
                curr_open = bar_dict[stock].open if stock in bar_dict else None
                ...
                if 0.5 <= open_pct <= 1.5:
                    # 获取当日收盘价和最高价（模拟）
                    curr_close = (
                        bar_dict[stock].close if stock in bar_dict else curr_open
                    )
                    curr_high = bar_dict[stock].high if stock in bar_dict else curr_open
    ...
    try:
        ...
        for stock in check_stocks:
            try:
                ...
                # 检查是否为二板（昨日和前两日都涨停）
                d0_close = prices[-1]["close"]  # 今日
                d0_limit = prices[-1]["high_limit"]
                ...
                # 大前日未涨停
                if len(prices) >= 4:
                    d3_close = prices[-4]["close"]
                    d3_limit = prices[-4]["high_limit"]
                    if abs(d3_close - d3_limit) / d3_limit < 0.005:
                        continue
                ...
                # 非涨停开盘
                curr_limit = bar_dict[stock].limit_up if stock in bar_dict else None
                if curr_limit and curr_open >= curr_limit * 0.99:
                    continue

                # 买入价和卖出价
                buy_price = curr_open * 1.005
                curr_high = bar_dict[stock].high if stock in bar_dict else curr_open
                curr_close = bar_dict[stock].close if stock in bar_dict else curr_open
...
def after_trading(context):
    ...
    # 每30天打印一次统计
    if len(context.combo_trades) % 30 == 0 and len(context.combo_trades) > 0:
        print(f"\n{'=' * 60}")
