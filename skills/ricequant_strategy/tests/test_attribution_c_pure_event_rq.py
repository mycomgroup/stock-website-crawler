"""
Tests for attribution_c_pure_event_rq.py
...
class TestInit:
    def test_init_sets_default_values(self, rq_globals):
        import importlib
        import attribution_c_pure_event_rq
        importlib.reload(attribution_c_pure_event_rq)
        context = MockContext()
        attribution_c_pure_event_rq.init(context)
...
class TestHandleBar:
    def test_handle_bar_no_position_finds_buy(self, rq_globals):
        ...
        context = MockContext()
        attribution_c_pure_event_rq.init(context)
        with patch.object(attribution_c_pure_event_rq, 'find_and_buy') as mock_find:
            attribution_c_pure_event_rq.handle_bar(context, {})
...
class TestFindAndBuy:
    def test_find_and_buy_no_stocks(self, rq_globals):
        ...
        importlib.reload(attribution_c_pure_event_rq)
        context = MockContext()
        attribution_c_pure_event_rq.find_and_buy(context, {})
    ...
    def test_find_and_buy_stock_not_trading(self, rq_globals):
        ...
        attribution_c_pure_event_rq.find_and_buy(context, bar_dict)
        rq_globals['order_target_percent'].assert_not_called()
