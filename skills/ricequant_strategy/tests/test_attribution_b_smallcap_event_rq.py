"""
Tests for attribution_b_smallcap_event_rq.py
Small cap + event (first board low open) strategy
"""
import sys
...
class TestInit:
    def test_init_sets_default_values(self, rq_globals):
        import importlib
        import attribution_b_smallcap_event_rq
        importlib.reload(attribution_b_smallcap_event_rq)
        context = MockContext()
        attribution_b_smallcap_event_rq.init(context)
...
class TestHandleBar:
    def test_handle_bar_no_position_finds_buy(self, rq_globals):
        ...
        context = MockContext()
        attribution_b_smallcap_event_rq.init(context)
        with patch.object(attribution_b_smallcap_event_rq, 'find_and_buy') as mock_find:
            attribution_b_smallcap_event_rq.handle_bar(context, {})
...
class TestFindAndBuy:
    def test_find_and_buy_no_stocks(self, rq_globals):
        ...
        importlib.reload(attribution_b_smallcap_event_rq)
        context = MockContext()
        attribution_b_smallcap_event_rq.find_and_buy(context, {})
        rq_globals['order_target_percent'].assert_not_called()

    def test_find_and_buy_market_cap_out_of_range(self, rq_globals):
        rq_globals['all_instruments'].return_value = pd.DataFrame({'order_book_id': ['000001.XSHE']})
        rq_globals['get_factor'].return_value = pd.DataFrame({'market_cap': [1.0]})
        import importlib
        import attribution_b_smallcap_event_rq
        importlib.reload(attribution_b_smallcap_event_rq)
        context = MockContext()
    ...
    def test_find_and_buy_stock_not_in_bar_dict(self, rq_globals):
        rq_globals['all_instruments'].return_value = pd.DataFrame({'order_book_id': ['000001.XSHE']})
        rq_globals['get_factor'].return_value = pd.DataFrame({'market_cap': [10.0]})
    ...
    def test_find_and_buy_stock_not_trading(self, rq_globals):
        ...
        attribution_b_smallcap_event_rq.find_and_buy(context, bar_dict)
        rq_globals['order_target_percent'].assert_not_called()

    def test_find_and_buy_open_out_of_range_high(self, rq_globals):
        rq_globals['all_instruments'].return_value = pd.DataFrame({'order_book_id': ['000001.XSHE']})
        rq_globals['get_factor'].return_value = pd.DataFrame({'market_cap': [10.0]})
        def mock_history_bars(code, count, freq, fields, date=None):
            if 'limit_up' in str(fields):
    ...
    def test_find_and_buy_open_out_of_range_low(self, rq_globals):
        rq_globals['all_instruments'].return_value = pd.DataFrame({'order_book_id': ['000001.XSHE']})
        rq_globals['get_factor'].return_value = pd.DataFrame({'market_cap': [10.0]})
        def mock_history_bars(code, count, freq, fields, date=None):
            if 'limit_up' in str(fields):
        ...
        importlib.reload(attribution_b_smallcap_event_rq)
        context = MockContext()
        bar_dict = {'000001.XSHE': MockBar(open_price=9.5)}
        attribution_b_smallcap_event_rq.find_and_buy(context, bar_dict)
        rq_globals['order_target_percent'].assert_not_called()
...
class TestEdgeCases:
    def test_find_and_buy_market_cap_lower_boundary(self, rq_globals):
        rq_globals['all_instruments'].return_value = pd.DataFrame({'order_book_id': ['000001.XSHE']})
        rq_globals['get_factor'].return_value = pd.DataFrame({'market_cap': [5.0]})
        def mock_history_bars(code, count, freq, fields, date=None):
            if 'limit_up' in str(fields):
                return np.array([(10.0, 10.0)], dtype=[('close', 'f8'), ('limit_up', 'f8')])
            return np.array([(10.0,)], dtype=[('close', 'f8')])
    ...
    def test_find_and_buy_market_cap_upper_boundary(self, rq_globals):
        rq_globals['all_instruments'].return_value = pd.DataFrame({'order_book_id': ['000001.XSHE']})
        rq_globals['get_factor'].return_value = pd.DataFrame({'market_cap': [15.0]})
        def mock_history_bars(code, count, freq, fields, date=None):
            if 'limit_up' in str(fields):
                return np.array([(10.0, 10.0)], dtype=[('close', 'f8'), ('limit_up', 'f8')])
            return np.array([(10.0,)], dtype=[('close', 'f8')])
    ...
    def test_find_and_buy_only_first_qualified(self, rq_globals):
        ...
        def mock_get_factor(stock, factor):
            return pd.DataFrame({'market_cap': [10.0]})
