...
try:
    ...
    universe = [
        s for s in universe if not any(s.startswith(p) for p in excluded_prefixes)
    ]
    ...
    def get_simple_factors(stock, date_str):
        ...
        roa_proxy = (close[-1] / close[-250] - 1) if len(close) >= 250 else 0

        cfoa_proxy = (
            np.mean(volume[-20:]) / np.mean(volume[-60:])
            if np.mean(volume[-60:]) > 0
            else 0
        )
    ...
    for stock in universe[:100]:
        try:
            factors = get_simple_factors(stock, "2024-01-01")
            if factors:
