...
def get_first_board_stocks(date):
    ...
    all_stocks = [
        s for s in all_stocks if not (s.startswith("688") or s.startswith("300"))
    ]
    all_stocks = [s for s in all_stocks if not (s.startswith("4") or s.startswith("8"))]
    ...
    for stock in all_stocks:
        if stock in prev_prices.minor_axis:
            close = prev_prices.loc["close", :, stock].iloc[0]
            high_limit = prev_prices.loc["high_limit", :, stock].iloc[0]
            if abs(close - high_limit) / high_limit < 0.01:
                zt_stocks.append(stock)

    prev_prev_date = get_previous_trading_date(prev_date)
    prev_prev_prices = get_price(
        zt_stocks,
    ...
    for stock in zt_stocks:
        if stock in prev_prev_prices.minor_axis:
            prev_close = prev_prev_prices.loc["close", :, stock].iloc[0]
            prev_high_limit = prev_prev_prices.loc["high_limit", :, stock].iloc[0]
...
def check_low_open(stock, date, prev_date):
    prices = get_price(
        stock, end_date=date, frequency="1d", fields=["open", "close"], count=2
    )

    if len(prices.major_axis) < 2:
        return None

    prev_close = prices.loc["close", prices.major_axis[0], stock]
    curr_open = prices.loc["open", prices.major_axis[1], stock]
...
def check_market_cap(stock, date):
    try:
...
def check_position(stock, date):
    try:
        prices = get_price(
            stock, end_date=date, frequency="1d", fields=["close"], count=15
        )

        if len(prices.major_axis) < 5:
            return False

        high_15d = max(prices.loc["close", :, stock])
        low_15d = min(prices.loc["close", :, stock])
        curr_close = prices.loc["close", prices.major_axis[-1], stock]
...
def calculate_returns(signal, date):
    try:
        ...
        next_high = next_prices.loc["high", next_prices.major_axis[0], signal["stock"]]
        next_close = next_prices.loc[
            "close", next_prices.major_axis[0], signal["stock"]
        ]
        next_open = next_prices.loc["open", next_prices.major_axis[0], signal["stock"]]
...
def run_period_analysis(period_name, start_date, end_date):
    print(f"\n{'=' * 60}")
    ...
    for i, date in enumerate(trading_dates):
        ...
        first_board_stocks = get_first_board_stocks(date)
        ...
        for stock in first_board_stocks[:50]:
            prev_date = get_previous_trading_date(date)
            signal = check_low_open(stock, date, prev_date)

            if signal:
                if check_market_cap(stock, date) and check_position(stock, date):
                    low_open_signals.append(signal)

        for signal in low_open_signals:
            returns = calculate_returns(signal, date)
            if returns:
...
for period_name, (start, end) in ALL_PERIODS:
    result = run_period_analysis(period_name, start, end)
    if result:
