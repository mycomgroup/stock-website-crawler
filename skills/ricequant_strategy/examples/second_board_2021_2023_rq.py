"""
任务04v2：二板2021-2023实测验证 - RiceQuant策略编辑器版本
修复版：处理空数据情况
"""
...
def quick_backtest_year(year, threshold=10):
    ...
    if results["trades"] > 0:
        ...
        results["pl_ratio"] = abs(avg_win / avg_loss) if avg_loss != 0 else 0
...
for yr in [2021, 2022, 2023]:
    ...
    if r.get("trades", 0) > 0:
        print(
            f"{yr}   {r.get('signals', 0)}   {r.get('trades', 0)}   {r.get('win_rate', 0):.1f}%  {r.get('avg_profit', 0):.2f}%  {r.get('avg_max_profit', 0):.2f}%  {r.get('pl_ratio', 0):.1f}  {r.get('cumulative', 0):.1f}%  {r.get('max_dd', 0):.1f}%"
        )
    else:
        print(f"{yr}   {r.get('signals', 0)}   {r.get('trades', 0)}   未完成")

win_rates = [r.get("win_rate", 0) for r in year_data.values() if r.get("trades", 0) > 0]
cumulative = [r.get("cumulative", 0) for r in year_data.values()]
