...
async function testSaveAndRead() {
  ...
  if (loginStatus.code !== 0) {
    console.error('✗ Not logged in. Run: node auto-login.js');
    process.exit(1);
  }
  ...
  // 3. 获取策略内容，验证代码是原始代码而非 base64
  console.log('\n3. Reading strategy content...');
  ...
  // 4. 更新策略，测试 saveStrategy
  console.log('\n4. Testing saveStrategy with new code...');
  const NEW_CODE = `
def init(context):
    context.counter = 0
...
`;
  ...
  // 6. 测试运行回测（不等待完成）
  console.log('\n6. Testing runBacktest (just start, no wait)...');
  try {
