...
export class MultiAccountManager {
  ...
  async addAccount(options = {}) {
    const { name, id, useStealth = false } = options;
    const accountId = id || `account-${Date.now()}`;

    console.log(`\n➕ 添加账号: ${name || accountId}`);

    // 捕获 session - use stealth mode if requested
    let sessionData;
    if (useStealth) {
      const { captureSessionStealth } = await import('./capture-session-stealth.js');
      sessionData = await captureSessionStealth(options);
    } else {
      sessionData = await captureSession(options);
    }
