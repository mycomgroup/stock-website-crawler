...
实验目录：skills/autoresearch_joinquant/strategy_autoresearch_<name>/
"""
...
from joinquant_executor import run_backtest, fetch_results, wait_for_completion
...
JQ_STRATEGY_DIR = AUTORESEARCH_DIR.parent / "joinquant_strategy"
CREATE_STRATEGY_JS = JQ_STRATEGY_DIR / "create_jq_strategy.js"
...
def create_jq_strategy(
    strategy_file: str, strategy_name: str, algorithm_id: str = None
) -> str:
    """调用 create_jq_strategy.js 或使用提供的 algorithm_id"""
    # 如果提供了 algorithm_id，直接返回
    if algorithm_id:
        print(f"[jq] 使用提供的 algorithmId={algorithm_id}")
        return algorithm_id

    if not CREATE_STRATEGY_JS.exists():
        raise FileNotFoundError(f"create_jq_strategy.js 不存在: {CREATE_STRATEGY_JS}")
    ...
    algorithm_id = json_output.get("strategy_id") or json_output.get("algorithmId")
    action = json_output.get("action", "")
    print(f"[jq] algorithmId={algorithm_id} ({action})")
    return algorithm_id
...
示例:
  ...
  python setup.py -f /path/to/strategy.py --name my_strategy
    ...
    parser.add_argument(
        "--algorithm-id",
        default=None,
        help="JoinQuant 策略 ID（algorithmId），如不提供则自动查找同名策略",
    )
    ...
    jq_strategy_name = f"autoresearch_jq_{strategy_name}"
    base = AUTORESEARCH_DIR / f"strategy_autoresearch_jq_{strategy_name}"

    print(f"[setup] 策略文件: {strategy_file}")
    print(f"[setup] 实验目录: {base}")
    print(f"[setup] JoinQuant 策略名: {jq_strategy_name}")
    ...
    # ── Step 1：在 JoinQuant 创建/复用策略 ────────────────────────────────
    print("\n[Step 1] 在 JoinQuant 创建/复用策略...")
    algorithm_id = create_jq_strategy(
        str(strategy_file), jq_strategy_name, args.algorithm_id
    )
    ...
    cfg = {
        ...
        "seed": {
            ...
            "strategy_name": jq_strategy_name,
        },
        "backtest": {
    ...
    print(f"  生成 seed_config.json (algorithmId={algorithm_id})")
    ...
    if src_readme.exists():
        ...
        content = content.replace(
            ...
            f"cat strategy_autoresearch_jq_{strategy_name}/history/iterations.tsv",
        )
        # 替换回测参数为实际值
        content = content.replace(
