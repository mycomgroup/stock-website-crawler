...
Usage:
    ...
    python analyze.py --base strategy_autoresearch_56_mlt_tsmom_20260407
"""
...
def section_next_suggestions(records: list[dict], state: dict) -> str:
    ...
    if keeps:
        ...
        if ir < 1.5:
            weak.append(f"IR 偏低({ir:.3f})，可尝试优化选股逻辑")
        if so < 2.0:
...
def main():
    parser = argparse.ArgumentParser(description="分析 autoresearch 实验结果")
    parser.add_argument("--base", required=True, help="实验目录路径")
    args = parser.parse_args()

    base = Path(args.base)
    if not base.is_absolute():
        # 支持相对路径，相对于 autoresearch 目录的父目录
