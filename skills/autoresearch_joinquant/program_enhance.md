...
- **优先尝试提升幅度大的改动**：score 提升 > 0.01 的方向值得深挖；提升 < 0.005 的方向尽量用参数微调而非引入新机制
- **效果不明显不堆规则**：若一个机制加入后 score 提升 < 0.003 且增加代码 > 20 行，rollback 并记录为"效果不足"
- **保持低复杂度**：每增加一个机制或参数，检查是否有简化的空间（删掉哪个子模块后得分不降？）
- **组合尝试 > 单维度深挖**：优先验证新方向（新机制），而不是在同一个机制上反复调参超过 3 轮
