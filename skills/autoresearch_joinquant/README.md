# Strategy Autoresearch (JoinQuant)

量化策略自动迭代优化系统。输入一个 JoinQuant 单文件策略，agent 自主循环迭代优化，全程留档可追溯。
...
├── joinquant_executor.py # JoinQuant HTTP API 封装
├── scorer.py             # 评分 + keep/rollback 决策
├── preflight_checker.py  # 策略文件预检查
├── program.md            # agent 行动规则
└── strategy_autoresearch_jq_<策略名>/   # 实验子目录
...
```bash
cd /Users/fengzhi/Downloads/git/testlixingren/skills/autoresearch_joinquant
...
脚本自动完成：
1. 在 JoinQuant 创建/复用 `autoresearch_jq_<name>` 策略，获取 algorithmId
...
```bash
cat strategy_autoresearch_jq_<name>/history/iterations.tsv
```
