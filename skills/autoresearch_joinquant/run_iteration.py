...
from joinquant_executor import (
    run_backtest,
    fetch_results,
    wait_for_completion,
...
def _git_rollback_to_champion(base: Path, champion_iter: str) -> None:
    ...
    if commit:
        ...
        print(
            f"[rollback] 警告：champion {champion_iter} 无 commit hash，strategy.py 未恢复",
            flush=True,
        )
    ...
    if current_iter > 0:
        ...
        if last_iter not in completed_iters:
            ...
            if champion_iter_str:
                ...
                print(
                    f"[recover] strategy.py 已恢复到 champion {champion_iter_str}",
                    flush=True,
                )
...
def main():
    ...
    _update_state(
        ...
        iter_n,
        decision,
        score if decision == "keep" else champion_score,
        iter_id if decision == "keep" else None,
        new_champion_metrics,
    )
