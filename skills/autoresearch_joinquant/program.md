...
**只读文件（不可修改）**：`seed_config.json`、`state.json`、`program.md`、`history/`、以及 autoresearch_joinquant 目录下的所有 `.py` 工具文件（`run_iteration.py`、`joinquant_executor.py`、`scorer.py`、`preflight_checker.py`）。
...
### 已验证有效（keep）
- [参数] top_n 30→20：score +0.08，集中持仓提升 calmar
- [参数] window 52→39：score +0.15，因子窗口缩短更敏感
- [仓位] exposure 100%→96%：score +0.02，轻微降低回撤

### 已验证无效（rollback）
- [参数] window 26, 35, 40, 42：均差于 39，39 是局部最优
- [参数] top_n 10, 12：股票池过小，回撤反而增大
- [调仓] 月度→季度：持仓漂移增大，回撤 +3%
- [过滤] PE<50：get_financials 接口不可用，crash
- [止损] 5% 止损：频繁触发，年化收益 -2%，score 下降

### 待探索方向
- [ ] 双月调仓（月度已试，季度失败，双月是中间点）
- [ ] 近期跌幅过滤（排除近 10 日跌幅 >15% 的股票，避免追跌）
- [ ] 因子加权方式（当前等权，可试 rank 加权）

### 规律总结
- 最优配置：window=39, top_n=20, exposure=96%
- 窗口 39 天（约 2 月）是当前因子最优回看期，过短噪声大，过长信号衰减
- 持仓 20 股集中效果好，低于 15 股回撤不稳定
- 96% 曝光度优于满仓，留少量现金缓冲有效
- PE 过滤因数据接口限制无法使用，不要再尝试 get_financials
- 止损在本策略中适得其反，因子本身已有反转保护，不要再加止损
```
