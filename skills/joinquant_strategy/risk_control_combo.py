...
def initialize(context):
    ...
    g.daily_pnl = 0
    g.stop_loss_count = 0
    g.weekly_start_value = 0
    g.weekly_pnl_pct = 0
    g.cool_down_days = 0
    g.week_start_date = None

    run_daily(init_weekly, "9:00")
    run_daily(get_stock_list, "9:01")
    run_daily(buy, "09:30")
    run_daily(sell_1030, "10:30")
    run_daily(sell_1330, "13:30")
    run_daily(sell_tail, "14:50")
    run_daily(record_daily_pnl, "15:00")
    run_daily(check_weekly_circuit_breaker, "15:01")


def init_weekly(context):
    current_date = context.current_dt.date()
    if g.week_start_date is None or (current_date - g.week_start_date).days >= 5:
        g.week_start_date = current_date
        g.weekly_start_value = context.portfolio.total_value
        g.weekly_pnl_pct = 0
        g.cool_down_days = 0


def get_stock_list(context):
    if g.cool_down_days > 0:
        g.target_list = []
        return
...
def record_daily_pnl(context):
    g.daily_pnl = context.portfolio.daily_pnl


def check_weekly_circuit_breaker(context):
    if g.weekly_start_value > 0:
        current_value = context.portfolio.total_value
        g.weekly_pnl_pct = (current_value - g.weekly_start_value) / g.weekly_start_value

        if g.weekly_pnl_pct < -0.08:
            if g.cool_down_days == 0:
                g.cool_down_days = 3
