...
class TestFactorTestMultiFactor:
    @pytest.fixture
    ...
    def test_initialize(self, setup_mocks):
        import factor_test_multi_factor as module
        ...
        assert setup_mocks['g'].max_cap == 50
        assert setup_mocks['g'].max_turnover == 30
    ...
    def test_get_stock_list_filters_by_cap(self, setup_mocks):
        import factor_test_multi_factor as module
        module.g = setup_mocks['g']
        module.g.max_cap = 50
        module.g.max_turnover = 30
        ...
        setup_mocks['jqlib'].HSL.return_value = [{'000001.XSHE': 20.0, '000002.XSHE': 25.0}]
        
        module.get_stock_list(setup_mocks['context'])
        
        assert '000003.XSHE' not in module.g.target_list

    def test_get_stock_list_filters_by_turnover(self, setup_mocks):
        import factor_test_multi_factor as module
        module.g = setup_mocks['g']
        module.g.max_cap = 50
        module.g.max_turnover = 25
        module.g.ps = 5
        ...
        setup_mocks['jqlib'].HSL.return_value = [{'000001.XSHE': 20.0, '000002.XSHE': 30.0}]
        
        module.get_stock_list(setup_mocks['context'])
        
        assert '000002.XSHE' not in module.g.target_list

    def test_get_stock_list_scoring_and_sorting(self, setup_mocks):
        import factor_test_multi_factor as module
        module.g = setup_mocks['g']
        module.g.max_cap = 50
        module.g.max_turnover = 30
        ...
        cap_data_all = pd.DataFrame({
            'code': ['000001.XSHE', '000002.XSHE', '000003.XSHE'],
            'circulating_market_cap': [20.0, 40.0, 30.0]
        })
        
        cap_data_1 = pd.DataFrame({'circulating_market_cap': [20.0]}, index=['000001.XSHE'])
        cap_data_2 = pd.DataFrame({'circulating_market_cap': [40.0]}, index=['000002.XSHE'])
        cap_data_3 = pd.DataFrame({'circulating_market_cap': [30.0]}, index=['000003.XSHE'])
        
        def mock_get_fundamentals(q, date):
            if hasattr(q, 'filter'):
                return cap_data_all
            if '000001.XSHE' in str(q):
                return cap_data_1
            elif '000002.XSHE' in str(q):
                return cap_data_2
            elif '000003.XSHE' in str(q):
                return cap_data_3
            return cap_data_1
        ...
        assert len(module.g.target_list) <= 2

    def test_get_stock_list_handles_exception(self, setup_mocks):
        import factor_test_multi_factor as module
        module.g = setup_mocks['g']
        module.g.max_cap = 50
        module.g.max_turnover = 30
    ...
    def test_multiple_positions(self, setup_mocks):
        import factor_test_multi_factor as module
        module.g = setup_mocks['g']
        module.g.ps = 3
        module.g.target_list = ['000001.XSHE', '000002.XSHE', '000003.XSHE']
        setup_mocks['context'].portfolio.available_cash = 100000
        setup_mocks['context'].portfolio.total_value = 100000
        
        module.buy(setup_mocks['context'])
        
        assert setup_mocks['jqdata'].order_value.call_count == 3

    def test_score_calculation_positive(self, setup_mocks):
        import factor_test_multi_factor as module
        ...
        setup_mocks['jqdata'].get_fundamentals.side_effect = mock_get_fundamentals
        setup_mocks['jqlib'].HSL.return_value = [{'000001.XSHE': 15.0, '000002.XSHE': 25.0}]
        ...
        def mock_get_fundamentals_full(q, date):
            if hasattr(q, 'filter') and hasattr(q.filter.call_args, '__getitem__'):
                return cap_data_all
            if '000001' in str(q):
                return cap_data_1
            return cap_data_2
        
        setup_mocks['jqdata'].get_fundamentals.side_effect = mock_get_fundamentals_full
        
        module.get_stock_list(setup_mocks['context'])
        
        assert len(module.g.target_list) > 0