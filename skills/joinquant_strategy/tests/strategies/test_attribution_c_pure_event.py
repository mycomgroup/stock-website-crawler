...
class TestAttributionCPureEvent:
    def test_initialize(self, mock_jqdata, mock_g):
        with patch('jqdata.g', mock_g):
            import attribution_c_pure_event
            mock_context = SimpleNamespace()
            attribution_c_pure_event.initialize(mock_context)
    ...
    def test_select_stocks_all_limit_up(self, mock_jqdata, mock_g, mock_context, mock_securities, mock_price_df_limit_up):
        with patch('jqdata.g', mock_g):
            import attribution_c_pure_event
            ...
            attribution_c_pure_event.select_stocks(mock_context)
    ...
    def test_buy_stocks_no_target(self, mock_jqdata, mock_g, mock_context):
        with patch('jqdata.g', mock_g):
            ...
            attribution_c_pure_event.buy_stocks(mock_context)
    ...
    def test_buy_stocks_open_pct_in_range(self, mock_jqdata, mock_g, mock_context):
        with patch('jqdata.g', mock_g):
            import attribution_c_pure_event
    ...
    def test_buy_stocks_open_pct_low_open(self, mock_jqdata, mock_g, mock_context):
        with patch('jqdata.g', mock_g):
            import attribution_c_pure_event
    ...
    def test_buy_stocks_open_pct_out_of_range_high(self, mock_jqdata, mock_g, mock_context):
        with patch('jqdata.g', mock_g):
            import attribution_c_pure_event
    ...
    def test_sell_stocks_with_positions(self, mock_jqdata, mock_g, mock_context):
        with patch('jqdata.g', mock_g):
            ...
            attribution_c_pure_event.sell_stocks(mock_context)
    ...
    def test_select_stocks_filters_st(self, mock_jqdata, mock_g, mock_context, mock_price_df_limit_up):
        with patch('jqdata.g', mock_g):
            import attribution_c_pure_event
            ...
            price_df = pd.DataFrame({
                ...
                'close': [10.0, 15.0],
                'high_limit': [10.0, 15.0]
            })
            mock_jqdata.get_price.return_value = price_df
            
            attribution_c_pure_event.select_stocks(mock_context)
            
            assert '000002.XSHE' not in mock_g.target
    ...
    def test_select_stocks_filters_new_stocks(self, mock_jqdata, mock_g, mock_context):
        with patch('jqdata.g', mock_g):
            import attribution_c_pure_event
            ...
            st_df = create_st_df(['000001.XSHE'], [False])
            mock_jqdata.get_extras.return_value = st_df
            
            attribution_c_pure_event.select_stocks(mock_context)
            
            mock_jqdata.get_price.assert_not_called()
            assert mock_g.target == []