...
class TestWeakToStrongV3Strict:
    def test_initialize(self, mock_jqdata, mock_g):
        with patch('jqdata.g', mock_g):
            import weak_to_strong_v3_strict
            context = SimpleNamespace()
            weak_to_strong_v3_strict.initialize(context)
    ...
    def test_filter_kcbj_stock(self, mock_jqdata, mock_g):
        with patch('jqdata.g', mock_g):
            ...
            result = weak_to_strong_v3_strict.filter_kcbj_stock(stocks)
            
            assert len(result) == 1
            assert '000001.XSHE' in result

    def test_buy_money_threshold_7billion(self, mock_jqdata, mock_g, mock_context):
        with patch('jqdata.g', mock_g):
            import weak_to_strong_v3_strict
            ...
            weak_to_strong_v3_strict.buy(mock_context)
            
            mock_jqdata.order_value.assert_not_called()

    def test_buy_market_cap_threshold_50billion(self, mock_jqdata, mock_g, mock_context):
        with patch('jqdata.g', mock_g):
            import weak_to_strong_v3_strict
            
            mock_g.target_list = ['000001.XSHE']
            
            prev_data = {'money': [8e8], 'volume': [1000000], 'close': [10.0]}
            mock_jqdata.attribute_history.return_value = prev_data
            
            mock_valuation = pd.DataFrame({'market_cap': [40]}, index=['000001.XSHE'])
            mock_jqdata.get_valuation.return_value = mock_valuation
    ...
    def test_buy_open_ratio_3_to_5_pct(self, mock_jqdata, mock_g, mock_context):
        with patch('jqdata.g', mock_g):
            import weak_to_strong_v3_strict
            ...
            mock_valuation = pd.DataFrame({'market_cap': [60]}, index=['000001.XSHE'])
            mock_jqdata.get_valuation.return_value = mock_valuation
    ...
    def test_buy_volume_threshold_1_0(self, mock_jqdata, mock_g, mock_context):
        with patch('jqdata.g', mock_g):
            import weak_to_strong_v3_strict
    ...
    def test_buy_volume_ratio_8_pct(self, mock_jqdata, mock_g, mock_context):
        with patch('jqdata.g', mock_g):
            import weak_to_strong_v3_strict
    ...
    def test_sell_not_at_limit(self, mock_jqdata, mock_g, mock_context):
        with patch('jqdata.g', mock_g):
            ...
            weak_to_strong_v3_strict.sell(mock_context)
