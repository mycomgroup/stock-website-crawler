...
def mock_jqdata_module():
    ...
    mock_module.math = __import__('math')
    return mock_module
...
def mock_g():
    g = SimpleNamespace()
    g.stock_pool = ['159915.XSHE', '510300.XSHG', '510500.XSHG']
    g.stock_num = 1
    g.momentum_day = 29
...
class TestStrategy05:
    ...
    def test_filter_paused_stock(self, mock_jqdata_module, mock_g):
        with patch.dict(sys.modules, {'jqdata': mock_jqdata_module}):
            import latest_strategy_05_8年10倍回撤小_有滑点_ETF动量简单轮动策略 as strategy
            mock_current_data = {
    ...
    def test_filter_st_stock(self, mock_jqdata_module):
        with patch.dict(sys.modules, {'jqdata': mock_jqdata_module}):
            ...
            mock_current_data = {
                ...
                '600000.XSHG': SimpleNamespace(is_st=False, name='浦发银行')
            }
            with patch.object(strategy, 'get_current_data', return_value=mock_current_data):
    ...
    def test_filter_limitup_stock(self, mock_jqdata_module, mock_context):
        with patch.dict(sys.modules, {'jqdata': mock_jqdata_module}):
            ...
            mock_current_data = {
                ...
                '510300.XSHG': SimpleNamespace(high_limit=20.0)
            }
            mock_history = {
                '159915.XSHE': [9.5],
                '510300.XSHG': [19.5]
            ...
            with patch.object(strategy, 'get_current_data', return_value=mock_current_data):
                with patch.object(strategy, 'history', return_value=mock_history):
                    stock_list = ['159915.XSHE', '510300.XSHG']
    ...
    def test_filter_limitdown_stock(self, mock_jqdata_module, mock_context):
        with patch.dict(sys.modules, {'jqdata': mock_jqdata_module}):
            ...
            mock_current_data = {
                ...
                '510300.XSHG': SimpleNamespace(low_limit=18.0)
            }
            mock_history = {
                '159915.XSHE': [9.5],
                '510300.XSHG': [19.5]
    ...
    def test_get_timing_signal_buy(self, mock_jqdata_module, mock_g):
        with patch.dict(sys.modules, {'jqdata': mock_jqdata_module}):
            ...
            mock_close_data.close = np.array([10.0, 10.5, 11.0, 11.5, 12.0] * 5)
            mock_high_low_data = SimpleNamespace()
            mock_high_low_data.low = np.array([9.0, 9.5, 10.0, 10.5, 11.0] * 4)
            mock_high_low_data.high = np.array([10.0, 10.5, 11.0, 11.5, 12.0] * 4)
    ...
    def test_check_lose_trigger(self, mock_jqdata_module, mock_context):
        with patch.dict(sys.modules, {'jqdata': mock_jqdata_module}):
            import latest_strategy_05_8年10倍回撤小_有滑点_ETF动量简单轮动策略 as strategy
            position = SimpleNamespace()
            position.security = '159915.XSHE'
