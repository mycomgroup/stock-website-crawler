...
// 使用新的算法ID（可以创建一个新的空白策略作为容器）
// 或者使用现有的策略ID
const ALGORITHM_ID = '309ebf2421687fcf4d41223fdec01f2c'; // 使用现有策略ID
...
async function main() {
  ...
  // 验证文件存在
  console.log('\n验证文件...');
  for (const strategy of USER_STRATEGIES) {
  ...
  for (let i = 0; i < USER_STRATEGIES.length; i++) {
    const result = await runSingleBacktest(client, context, USER_STRATEGIES[i], i);
    results.push(result);
  ...
  // 保存结果到文件
  const outputPath = path.join(client.outputRoot || __dirname, `user-attribution-${Date.now()}.json`);
