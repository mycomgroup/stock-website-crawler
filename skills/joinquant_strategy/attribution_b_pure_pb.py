...
def initialize(context):
    ...
    g.release_name = "RFScore7_Attribution_B_Pure_PB"
    g.ipo_days = 180
...
def calc_valuation_table(stocks, watch_date):
    val = get_valuation(
    ...
    val = val.dropna(subset=["pb_ratio", "pe_ratio"])
    val = val[
        (val["pb_ratio"] > 0)
        & (val["pe_ratio"] > 0)
        & (val["pe_ratio"] < g.max_pe_ratio)
    ].copy()
...
def sort_by_pb(df):
    if df.empty:
        return df
    return df.sort_values("pb_ratio", ascending=True)
...
def choose_stocks(watch_date, target_hold_num):
    ...
    df = calc_valuation_table(stocks, str(watch_date))
    ...
    # 纯PB选股：只选PB最低10%
    primary = sort_by_pb(
        df[df["pb_group"] == g.primary_pb_group].copy()
    )
...
def rebalance(context):
    ...
    log.info(
        "release=%s watch_date=%s breadth=%.3f trend_on=%s target_hold_num=%s actual_hold_num=%s cash_ratio=%.2f"
        % (
            g.release_name,
            str(watch_date),
