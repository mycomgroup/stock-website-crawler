...
export async function analyzeGuornAPI(options = {}) {
  ...
  page.on('request', request => {
    ...
    if (url.includes('guorn.com') && !url.includes('.css') && !url.includes('.js') && !url.includes('.png') && !url.includes('.jpg') && !url.includes('.gif')) {
      apiCalls.push({
        type: 'request',
  ...
  try {
    // Look for password login tab/button
  ...
  const apiCallsFile = path.join(DATA_ROOT, 'api-calls.json');
  ...
  await browser.close();
  return { apiCalls, sessionPayload };
}

if (process.argv[1] && import.meta.url === `file://${process.argv[1]}`) {
  analyzeGuornAPI({ headed: process.argv.includes('--headed') }).catch(console.error);
}
