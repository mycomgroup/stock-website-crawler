...
export async function captureBacktestAPI(options = {}) {
  ...
  console.log('Clicking backtest button...');
  const backtestButton = page.locator('a:has-text("开始回测"), button:has-text("开始回测")').first();
...
if (process.argv[1] && import.meta.url === `file://${process.argv[1]}`) {
  captureBacktestAPI({ headed: process.argv.includes('--headed') }).catch(console.error);
}
