...
python ${SKILL_DIR}/run_iteration.py \
  ...
  --mutation-summary "I_portfolio_risk 单轮验证"
```
...
- 不要并行跑多个迭代
- I_portfolio_risk 默认只做有限粗搜索和有限细调
...
```bash
cat skills/autoresearch_10jqka_backtest/trees/I_portfolio_risk/experiments/<name>/state.json
cat skills/autoresearch_10jqka_backtest/trees/I_portfolio_risk/experiments/<name>/iterations.tsv | tail -5
```
