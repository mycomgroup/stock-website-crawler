...
python ${SKILL_DIR}/run_iteration.py \
  ...
  --mutation-summary "E_technical_trend 单轮验证"
```
...
- 不要并行跑多个迭代
- E_technical_trend 默认只做有限粗搜索和有限细调
...
```bash
cat skills/autoresearch_10jqka_backtest/trees/E_technical_trend/experiments/<name>/state.json
cat skills/autoresearch_10jqka_backtest/trees/E_technical_trend/experiments/<name>/iterations.tsv | tail -5
```
