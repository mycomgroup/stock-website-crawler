...
python ${SKILL_DIR}/run_iteration.py \
  ...
  --mutation-summary "B_quality_growth 单轮验证"
```
...
- 不要并行跑多个迭代
- B_quality_growth 默认只做有限粗搜索和有限细调
...
```bash
cat skills/autoresearch_10jqka_backtest/trees/B_quality_growth/experiments/<name>/state.json
cat skills/autoresearch_10jqka_backtest/trees/B_quality_growth/experiments/<name>/iterations.tsv | tail -5
```
