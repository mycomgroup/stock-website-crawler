...
def load_history(base: Path) -> list[dict]:
    ...
    for f in files:
        try:
            ...
            d["_sortino"] = d.get("sortino") or 0
            d["_ir"] = d.get("information_ratio") or 0
            records.append(d)
        except Exception:
            pass
    return records
...
def section_overview(records: list[dict], state: dict) -> str:
    ...
    baseline = next((r for r in records if "0000" in r.get("iter", "")), None)
...
def main():
    parser = argparse.ArgumentParser(description="分析果仁策略实验结果")
    ...
    header = f"# 果仁策略 Autoresearch 分析报告\n生成时间: {now}\n实验目录: {base.name}\n"
