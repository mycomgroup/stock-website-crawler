"""
最近两年（2024-2025）数据分析和回测脚本

基于已下载的weekly_factors数据，分析最近两年的因子表现和策略效果。

使用方法：
    python skills/autoresearch_ml_joinquant_factor_v2/analyze_recent_2years.py

输出文件：
    - output/analysis/recent_2years_report.txt (文本报告)
    - output/analysis/recent_2years_summary.json (JSON摘要)
"""
...
class Recent2YearsAnalyzer:
    ...
    def load_recent_data(self):
        """加载最近两年的weekly_factors数据"""
        logger.info("=" * 80)
        logger.info("加载最近两年数据...")
        logger.info("=" * 80)
        
        # 获取2024和2025年的所有数据文件
        all_files = sorted(glob.glob(str(DATA_DIR / "factors_*.csv")))
        self.data_files = [f for f in all_files if "factors_2024" in f or "factors_2025" in f]
        
        if not self.data_files:
            raise FileNotFoundError(f"未找到2024-2025年的数据文件: {DATA_DIR}")
    ...
    def preprocess_factors(self):
        ...
        logger.info("  执行截面标准化...")
        self.df_all = self.df_all.groupby('date').apply(standardize_cross_section)
    ...
    def backtest_strategy(self):
        ...
        self.portfolio_returns = self.df_all.groupby('date').apply(simulate_top50)
        ...
        n_years = date_range.days / 365.25
        
        # 使用实际年数计算年化收益
        ann_port = ((port_cum.iloc[-1]) ** (1 / n_years) - 1) * 100
        ann_mkt = ((mkt_cum.iloc[-1]) ** (1 / n_years) - 1) * 100
        ann_vol_port = self.portfolio_returns.std() * np.sqrt(52) * 100
        ann_vol_mkt = self.market_returns.std() * np.sqrt(52) * 100
        
        logger.info(f"  数据跨度: {n_years:.2f}年 ({n_weeks}周)")
        logger.info(f"  组合累计收益: {(port_cum.iloc[-1] - 1) * 100:.2f}%")
        logger.info(f"  组合年化收益: {ann_port:.2f}%")
        logger.info(f"  组合年化波动: {ann_vol_port:.2f}%")
        logger.info(f"  组合夏普比率: {ann_port / ann_vol_port:.2f}")
        logger.info(f"  市场累计收益: {(mkt_cum.iloc[-1] - 1) * 100:.2f}%")
    ...
    def generate_report(self) -> str:
        ...
        report_lines.append("最近两年（2024-2025）因子分析和回测报告")
        ...
        df_groups = self.df_all.groupby('date').apply(calc_10groups)
        group_returns = df_groups.groupby('group')['pchg'].mean() * 100
        ...
        report_lines.append(f"数据跨度: {n_years:.2f}年 ({n_weeks}周)")
        report_lines.append("")
        ...
        monthly_port = self.portfolio_returns.groupby(self.portfolio_returns.index.to_period('M')).sum()
        monthly_mkt = self.market_returns.groupby(self.market_returns.index.to_period('M')).sum()
        report_lines.append(f"  月胜率: {(monthly_port > monthly_mkt).mean() * 100:.2f}%")
        report_lines.append("")
        ...
        # 使用复利计算年度收益
        annual_port = self.portfolio_returns.groupby(self.portfolio_returns.index.year).apply(lambda x: (1 + x).prod() - 1)
        annual_mkt = self.market_returns.groupby(self.market_returns.index.year).apply(lambda x: (1 + x).prod() - 1)
        ...
        for year in sorted(annual_port.index):
            ...
            year_returns = self.portfolio_returns[self.portfolio_returns.index.year == year]
            year_sharpe = year_returns.mean() / year_returns.std() * np.sqrt(52) if year_returns.std() > 0 else 0
            
            report_lines.append(f"{year} | {port_y:>7.2f}% | {mkt_y:>7.2f}% | {excess_y:>7.2f}% | {year_sharpe:>6.2f}")
        report_lines.append("")
        
        # 结论
        report_lines.append("【六、结论】")
        report_lines.append("")
        report_lines.append("最近两年表现:")
    ...
    def save_results(self, report_text: str, summary_json: dict):
        ...
        # 保存文本报告
        report_path = ANALYSIS_DIR / "recent_2years_report.txt"
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write(report_text)
        logger.info(f"\n文本报告已保存: {report_path}")
        
        # 保存JSON摘要
        summary_path = ANALYSIS_DIR / "recent_2years_summary.json"
