...
def get_price_for_date(date: str, duckdb_path: str = DUCKDB_PATH, max_retries: int = 5) -> Dict[str, float]:
    """
    从 DuckDB 获取指定日期的股票价格
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            # 查询价格数据（表名: daily_data, 列名: symbol, close）
            query = f"""
                SELECT symbol, close 
                FROM daily_data 
                WHERE date = '{date}'
            ...
            # 转换为字典
            price_dict = {str(row[0]): float(row[1]) for row in result if row[1] is not None}
            
            LOGGER.info(f"  获取价格: {len(price_dict)} 只股票")
            return price_dict
            
        except Exception as e:
            if attempt < max_retries - 1:
                LOGGER.warning(f"  获取价格失败 (尝试 {attempt + 1}/{max_retries}): {e}")
                time.sleep(2)  # 等待2秒后重试
            else:
                LOGGER.error(f"  获取价格失败，已达最大重试次数: {e}")
                return {}
        finally:
            if conn:
                conn.close()
        ...
        while current_dt <= end_dt:
            ...
            if download_file(oss_path, str(temp_path)):
                try:
                    ...
                    if price_dict:
                        # 转换股票代码格式后匹配价格
                        df[price_column_name] = df[stock_col].apply(lambda x: price_dict.get(convert_stock_code(x)))
                        
                        # 统计匹配情况
                        matched = df[price_column_name].notna().sum()
                        total = len(df)
                        match_rate = matched / total * 100 if total > 0 else 0
                        LOGGER.info(f"  价格匹配: {matched}/{total} ({match_rate:.1f}%)")
                    else:
            ...
            if download_file(oss_path, str(temp_path)):
                try:
                    ...
                    if price_this_week and price_next_week:
                        ...
                        # 调整列顺序：Unnamed: 0, date, pchg, ...其他因子列
                        cols = [stock_col, 'date', 'pchg'] + [c for c in df.columns if c not in [stock_col, 'date', 'pchg']]
                        df = df[cols]
