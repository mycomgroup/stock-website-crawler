...
策略：
1. 第一阶段：先取预测值Top 10%的股票（预测效果最好的）
2. 第二阶段：在这10%中，按不同条件选Top N
...
def calculate_two_stage_portfolio(
    predictions,
    actual_data,
    n_stocks=50,
    top_pct=0.1,  # 第一阶段：取Top 10%
    stage2_filter='none',  # 第二阶段过滤方式
    pred_col='prediction'
):
    """
    两阶段选股
    ...
    stage2_filter : str
        ...
        - 'none': 直接选Top N
        - 'eps0': EPS>0 + Top N
        - 'eps0.1': EPS>0.1 + Top N
        - 'eps0.5': EPS>0.5 + Top N
        - 'small_cap_eps0.1': 小市值 + EPS>0.1 + Top N
    """
    ...
    def simulate_portfolio(group):
        # 第一阶段：选取预测值Top 10%
        n_stage1 = max(int(len(group) * top_pct), n_stocks)  # 至少要有n_stocks只
        stage1_pool = group.nlargest(n_stage1, pred_col)
        
        # 第二阶段：根据不同策略选股
        if stage2_filter == 'none':
            # 直接选Top N（已经是Top了，取前N个）
            final_pool = stage1_pool.head(n_stocks)
        
        elif stage2_filter == 'eps0':
            # EPS>0 + Top N
            filtered = stage1_pool[stage1_pool['eps'] > 0]
            if len(filtered) < n_stocks:
                final_pool = filtered
            else:
                final_pool = filtered.nlargest(n_stocks, pred_col)
        
        elif stage2_filter == 'eps0.1':
            # EPS>0.1 + Top N
            filtered = stage1_pool[stage1_pool['eps'] > 0.1]
            if len(filtered) < n_stocks:
                final_pool = filtered
            else:
                final_pool = filtered.nlargest(n_stocks, pred_col)
        
        elif stage2_filter == 'eps0.5':
            # EPS>0.5 + Top N
            filtered = stage1_pool[stage1_pool['eps'] > 0.5]
            if len(filtered) < n_stocks:
                final_pool = filtered
            else:
                final_pool = filtered.nlargest(n_stocks, pred_col)
        
        elif stage2_filter == 'small_cap_eps0.1':
            # 小市值 + EPS>0.1 + Top N
            filtered = stage1_pool[stage1_pool['eps'] > 0.1]
            filtered = filtered.sort_values('circulating_market_cap', ascending=True)
            if len(filtered) < n_stocks:
                final_pool = filtered
            else:
                final_pool = filtered.nlargest(n_stocks, pred_col)
...
def generate_report(results_df):
    """生成对比报告"""
    logger.info("\n" + "=" * 140)
    logger.info("两阶段选股策略对比")
    logger.info("=" * 140)
    
    report_lines = []
    report_lines.append("=" * 140)
    report_lines.append("两阶段选股策略对比")
    report_lines.append("=" * 140)
    report_lines.append("")
    
    report_lines.append("【策略说明】")
    report_lines.append("")
    report_lines.append("第一阶段：先取预测值Top 10%的股票（预测效果最好的）")
    report_lines.append("第二阶段：在这10%中，按不同条件选Top 50")
    report_lines.append("")
    report_lines.append("测试的第二阶段策略：")
    report_lines.append("  1. 直接选Top 50（按预测值）")
    report_lines.append("  2. EPS>0 + Top 50")
    report_lines.append("  3. EPS>0.1 + Top 50")
    report_lines.append("  4. EPS>0.5 + Top 50")
    report_lines.append("  5. 小市值 + EPS>0.1 + Top 50")
    report_lines.append("")
    
    report_lines.append("【详细结果】")
    report_lines.append("")
    report_lines.append(f"{'策略描述':<35} | {'年化收益':<10} | {'年化波动':<10} | {'夏普比率':<10} | {'最大回撤':<10} | {'胜率':<8} | {'年化超额':<10}")
    report_lines.append("-" * 140)
    
    for _, row in results_df.iterrows():
        report_lines.append(
            f"{row['strategy_name']:<35} | "
    ...
    # 对比单阶段策略
    report_lines.append("3. 两阶段 vs 单阶段对比:")
    report_lines.append("")
    report_lines.append("   单阶段最优（EPS>0.1直接选50支）:")
    report_lines.append("     年化收益: 20.38%, 夏普: 0.86")
    report_lines.append("")
    report_lines.append(f"   两阶段最优（{best_sharpe_row['strategy_name']}）:")
    report_lines.append(f"     年化收益: {best_sharpe_row['annual_return']:.2f}%, 夏普: {best_sharpe_row['sharpe']:.2f}")
    report_lines.append("")
    
    improvement = best_sharpe_row['annual_return'] - 20.38
    if improvement > 1:
        report_lines.append(f"   ✅ 两阶段策略显著优于单阶段（+{improvement:.2f}pp）")
    elif improvement > 0:
        report_lines.append(f"   ⚠️ 两阶段策略略优于单阶段（+{improvement:.2f}pp）")
    else:
        report_lines.append(f"   ❌ 两阶段策略未改善收益（{improvement:.2f}pp）")
    ...
    # 保存报告
    report_path = OUTPUT_DIR / 'analysis' / 'two_stage_selection_analysis.txt'
...
def main():
    ...
    try:
        ...
        # 定义测试策略
        test_strategies = [
            {'name': '两阶段：Top10% → 直接选Top50', 'filter': 'none'},
            {'name': '两阶段：Top10% → EPS>0 + Top50', 'filter': 'eps0'},
            {'name': '两阶段：Top10% → EPS>0.1 + Top50', 'filter': 'eps0.1'},
            {'name': '两阶段：Top10% → EPS>0.5 + Top50', 'filter': 'eps0.5'},
            {'name': '两阶段：Top10% → 小市值+EPS>0.1+Top50', 'filter': 'small_cap_eps0.1'},
        ]
        
        logger.info("\n" + "=" * 80)
        logger.info("开始测试两阶段选股策略")
        logger.info("=" * 80)
        ...
        for strategy in test_strategies:
            ...
            perf = calculate_two_stage_portfolio(
                phase2_pred,
                df_2026,
                n_stocks=50,
                top_pct=0.1,
                stage2_filter=strategy['filter'],
                pred_col='prediction'
            )
        ...
        # 保存详细结果
        csv_path = OUTPUT_DIR / 'analysis' / 'two_stage_selection_results.csv'
