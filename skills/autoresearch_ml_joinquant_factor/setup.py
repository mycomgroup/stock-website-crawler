...
def _default_dates() -> tuple[str, str]:
    ...
    start = (today - timedelta(days=365 * 8)).strftime("%Y-%m-%d")
    return start, end
...
def score_from_metrics(metrics, turnover_mean, net_penalty):
    # 保守打分：收益 + 稳定性 - 交易摩擦
    raw = (
        0.25 * np.tanh(metrics["long_short_return"] / 8.0)
        + 0.20 * np.tanh(metrics["top_sharpe"] / 4.0)
        + 0.20 * np.tanh(metrics["ic_mean"] / 0.08)
        + 0.20 * np.tanh(metrics["icir"] / 2.0)
...
def main():
    ...
    for i, date in enumerate(dates[:-1]):
        try:
            ...
            px = get_price(stocks, date, next_date, "1d", "close")["close"]
            if px.shape[0] < 2:
                continue

            fdf["future_ret"] = px.iloc[-1] / px.iloc[1] - 1
            fdf["date"] = date
            rows.append(fdf)
        except:
            pass
    ...
    y_test = test["future_ret"]

    # 模型训练
    model = BayesianRidge()
    model.fit(X_train, y_train)
    test["pred_raw"] = np.dot(X_test, model.coef_)
    ...
    score = score_from_metrics(metrics, turnover_mean=turnover_mean, net_penalty=net_penalty)
...
def run_baseline(strategy_file: Path, notebook_dir: str | None):
    ...
    result = execute_strategy(str(strategy_file), timeout_ms=600_000, notebook_dir=notebook_dir)
...
def main():
    ...
    parser.add_argument("--start-date", default=DEFAULT_START, help=f"默认 {DEFAULT_START}")
    parser.add_argument("--end-date", default=DEFAULT_END, help=f"默认 {DEFAULT_END}")
    parser.add_argument("--pool", default="small", choices=["small", "HS300", "ZZ500", "ZZ800"], help="选股池")
    ...
    parser.add_argument("--ic-window", type=int, default=26, help="IC rolling窗口（周频约半年）")
    parser.add_argument("--cluster-count", type=int, default=20, help="聚类簇数（预留参数）")
    ...
    ], cwd=str(exp_dir), capture_output=True)

    src_program = AUTORESEARCH_DIR / "program.md"
    if src_program.exists():
        (exp_dir / "program.md").write_text(src_program.read_text(encoding="utf-8"), encoding="utf-8")
