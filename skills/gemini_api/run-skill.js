...
function showHelp() {
  console.log(`
Google Gemini API Skill - 程序化提交问题到 Gemini
...
选项:
  ...
  --model <name>       指定模型 (gemini-pro, gemini-pro-vision 等)
  ...
  --show <id>          显示特定对话的详细内容
  --delete <id>        删除特定对话
  --models             列出可用模型 (Gemini Pro, Gemini Pro Vision)
  --account            显示账户信息
...
示例:
  ...
  # 指定模型
  node run-skill.js --message "写一个快速排序算法" --model gemini-pro
  ...
  # 使用现有 Chrome profile
  node run-skill.js --use-profile "/Users/username/Library/Application Support/Google/Chrome/Default"
`);
}
...
async function main() {
  ...
  try {
    ...
    if (args.list || args['list-all']) {
      const { GeminiClient } = await import('./request/gemini-client.js');
    ...
    // Note: Rename and Clear features removed for Gemini
    // (These features may not be supported by Gemini API)
