...
| 参数名称 | 类型 | 必选 | 说明 |
| -------- | ---- | ---- | ---- |
| ticker | string | 否 | Specify an option contract or use * to subscribe to all option contracts.<br>For business plans, the wildcard option is only available on the Full Market and Full Market Delayed expansions.<br>You can also use a comma separated list to subscribe to multiple option contracts.<br>You can retrieve active options contracts from our Options Contracts API. |

## Response Attributes

| 参数名称 | 类型 | 必选 | 说明 |
| -------- | ---- | ---- | ---- |
| ev | enum (AM) | 否 | The event type. |
...
{"action":"subscribe", "params":"AM.O:SPY251219C00650000"}
```
