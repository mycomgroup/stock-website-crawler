...
```bash
# 保持 session 运行（用于调试）
node run-strategy.js --strategy examples/test_mini.py --no-shutdown

# 或显式指定
node run-strategy.js --strategy examples/test_mini.py --auto-shutdown false
```
...
# 手动查看 https://www.joinquant.com/research
# 找到 "运行中" 查看状态

# 完成后手动关闭或等待自动超时
```

### 场景 3：关闭所有运行中的 notebook
...
JoinQuant 使用 JupyterHub + Jupyter Notebook 架构，API 符合标准：
...
- [REST API 参考](https://jupyter-server.readthedocs.io/en/latest/operators/rest-api.html)
- [Session 管理](https://jupyter-notebook.readthedocs.io/en/stable/computing.html)