#!/usr/bin/env python3
"""
任务03-提示词3.2：止盈规则测试（JoinQuant Notebook格式）
测试期间：2024年Q1
"""

print("=" * 70)
print("任务03-提示词3.2：止盈规则测试（JoinQuant Notebook）")
print("=" * 70)
...
try:
    ...
    results = {
        "无止盈": {"trades": 0, "wins": 0, "profits": [], "triggered": 0},
        "+5%止盈": {"trades": 0, "wins": 0, "profits": [], "triggered": 0},
        "+10%止盈": {"trades": 0, "wins": 0, "profits": [], "triggered": 0},
    }
    ...
    for i in range(2, min(len(test_dates) - 1, 50)):
        ...
        try:
            ...
            for rule in ["无止盈", "+5%止盈", "+10%止盈"]:
                sell_price = close_price
                profit_pct = (sell_price - buy_price) / buy_price * 100
                triggered = False

                if rule == "+5%止盈" and high_price >= buy_price * 1.05:
                    sell_price = buy_price * 1.05
                    profit_pct = 5.0
                    triggered = True
                elif rule == "+10%止盈" and high_price >= buy_price * 1.10:
    ...
    for rule, data in results.items():
        if data["trades"] > 0:
            ...
            print(
                ...
                f"{avg_profit:.2f}% | {trigger_rate:.1f}% |"
            )
