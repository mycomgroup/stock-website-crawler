...
skips = 0
for i, (d, nd) in enumerate(zip(test_dates, next_dates)):
    stocks, br, reason = select_enhanced(d)
    if stocks:
        r = calc_return(stocks, d, nd)
        enh_rets.append(r)
        print(f"  {d}: 广度={br:.2f} {reason} {len(stocks)}只, 收益={r * 100:.2f}%")
    else:
        enh_rets.append(0)
        skips += 1
        print(f"  {d}: 广度={br:.2f} -> 清仓")

# 结果
print("\n" + "=" * 50)
print("结果对比：")
print(
    f"  原始策略: 累计={(1 + pd.Series(base_rets)).prod() - 1:.2%}, 月均={np.mean(base_rets) * 100:.2f}%"
)
print(
    f"  增强策略: 累计={(1 + pd.Series(enh_rets)).prod() - 1:.2%}, 月均={np.mean(enh_rets) * 100:.2f}%"
)
print(f"  清仓次数: {skips}")
print("=" * 50)
