...
for i in range(1, len(trade_days) - 1):
    ...
    try:
        ...
        valuations = get_valuation(
            stock_codes, end_date=prev_str, fields=["market_cap"]
        )
        if not valuations.empty:
            valuations = valuations.dropna()
            valuations = valuations[
                (valuations["market_cap"] >= 10) & (valuations["market_cap"] <= 30)
            ]
            stock_codes = list(valuations["code"])
