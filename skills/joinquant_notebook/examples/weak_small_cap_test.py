...
for i in range(1, len(trade_days) - 1):
    ...
    try:
        ...
        vals = get_valuation(stock_codes, end_date=prev_str, fields=["market_cap"])
        if not vals.empty:
            vals = vals.dropna()
            vals = vals[(vals["market_cap"] >= 10) & (vals["market_cap"] <= 30)]
            stock_codes = list(vals["code"])
