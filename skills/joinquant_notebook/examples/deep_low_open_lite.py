...
print("开始深度低开专项验证分析（高效采样版）...")
print("=" * 80)

test_dates_by_year = {
    2021: ["2021-03-01", "2021-06-01", "2021-09-01", "2021-12-01"],
    2022: ["2022-03-01", "2022-06-01", "2022-09-01", "2022-12-01"],
    2023: ["2023-03-01", "2023-06-01", "2023-09-01", "2023-12-01"],
    2024: ["2024-01-02", "2024-04-01", "2024-07-01", "2024-10-01"],
}

signals_by_year = {}
total_zt_by_year = {}

for year, dates in test_dates_by_year.items():
    print(f"\n处理 {year} 年数据（采样{len(dates)}个关键月份）...")
    year_signals = []
    year_zt_count = 0
    ...
    for curr_date in dates:
        try:
            ...
            print(f"  {curr_date}: 涨停板{len(limit_stocks)}个")
...
if len(all_signals) == 0:
    ...
    if len(deep_low_all) >= 30:
        ...
        if len(deep_low_all) > 0:
            ...
            if avg_return < 0 and win_rate < 30:
                print("\n判定：删除")
                print("理由：样本虽少但收益负且胜率极低，应删除")
            else:
                print("\n判定：暂不删除，需补充更多样本")
                print("理由：样本不足，无法做出可靠判断")
        else:
            print("\n判定：删除")
            print("理由：无任何深度低开样本，不存在该结构")
