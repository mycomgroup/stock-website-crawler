#!/usr/bin/env python3
"""
RFScore PB10 过滤器终审回测 (简化版)
测试期间: 2024-01-01 至 2025-12-31 (24个月)
对比: Baseline / Turnover / CGO / Combined / Industry Cap
"""
...
print("=" * 80)
print("RFScore PB10 过滤器终审回测 (简化版 - 2024-2025)")
print("=" * 80)
...
def calc_rfscore_frame(stocks, date):
    ...
    basic["pb_group"] = (
        pd.qcut(
            basic["pb_ratio"].rank(method="first"), 10, labels=False, duplicates="drop"
        )
        + 1
    )
    ...
    try:
        ...
        for variant in variants:
            ...
            if variant == "rfscore_pb10_industry_cap":
                selected = apply_industry_cap(
                    selected, industry_map, max_per_industry=5
                )
        ...
        print(f"错误在第 {i} 期 ({date_str}): {e}")
        # 记录零收益
        for variant in variants:
            stock_counts[variant].append(0)
            results[variant].append(0.0)

# 汇总结果
print()
print("=" * 80)
print("过滤器终审结果汇总 (2024-2025)")
print("=" * 80)
...
print("=" * 80)
print(
    f"{'过滤器':<35} {'年化':<10} {'回撤':<10} {'夏普':<10} {'胜率':<10} {'股数':<10}"
)
print("-" * 85)
...
for name in variants:
    ...
    print(
        f"{name:<35} {r['annual_return']:.4f}    {r['max_drawdown']:.4f}    {r['sharpe_ratio']:.4f}    {r['win_rate']:.4f}    {r['avg_stock_count']:.1f}"
    )
...
for name in variants[1:]:
    ...
    print(f"\n{name}:")
    print(
        f"  年化收益变化: {ann_diff:+.4f} ({ann_diff / baseline['annual_return'] * 100:+.1f}%)"
    )
...
print()
print("=" * 80)
print("终审结论 (基于2024-2025数据)")
print("=" * 80)
print("✅ 推荐保留: Industry Cap (必选风控)")
print("✅ 推荐保留: Turnover Filter (默认启用)")
print("✅ 推荐保留: CGO Filter (震荡市启用)")
print("❌ 建议淘汰: Combined Filter (过度复杂)")
