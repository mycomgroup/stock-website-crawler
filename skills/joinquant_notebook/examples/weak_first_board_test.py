...
for i in range(1, len(trade_days) - 1):
    ...
    prev2_str = prev2_date.strftime("%Y-%m-%d")
    ...
    try:
        ...
        prev2_prices = get_price(stocks, end_date=prev2_str, frequency="daily",
                                fields=["close", "high_limit"], count=1, panel=False)
        if not prev2_prices.empty:
            prev2_prices = prev2_prices.dropna()
            prev2_hl = set(prev2_prices[prev2_prices["close"] == prev2_prices["high_limit"]]["code"])
            hl_df = hl_df[~hl_df["code"].isin(prev2_hl)]
