...
print("=" * 80)
print("任务05v2：卖出规则深度对比测试（真实数据优化版）")
print("=" * 80)
...
for i in range(len(all_trade_days) - 1):
    if processed >= max_signals:
        print(f"已达到最大信号数 {max_signals}，停止筛选")
        break
    ...
    try:
        print(f"处理日期: {prev_date_str}...", end=" ")
        ...
        zt_stocks = prices[prices["close"] >= prices["high_limit"] * 0.995][
            "code"
        ].tolist()
        ...
        try:
            ...
            for stock in zt_stocks[:20]:
                try:
                    ...
                    if 0.005 <= open_change <= 0.015:
                        if not any(
                            s["stock"] == stock and s["date"] == date_str
                            for s in signals
                        ):
                            signals.append(
                                {
...
exec_analysis = {
    "S1": {"exec": True, "diff": "最容易"},
    "S2": {"exec": True, "diff": "容易"},
    "S3": {"exec": True, "diff": "容易"},
    "S4": {"exec": True, "diff": "中等"},
    "S5": {"exec": True, "diff": "较难"},
    "S6": {"exec": True, "diff": "中等"},
    "S7": {"exec": False, "diff": "不可执行"},
}
...
for rule_id in ["S1", "S2", "S3", "S4", "S5", "S6", "S7"]:
    if rule_id in summary:
        ...
        print(f"  {rule_id} {summary[rule_id]['name']:<18}: {status} ({a['diff']})")
