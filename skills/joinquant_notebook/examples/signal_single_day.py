...
prev_idx = trade_days.tolist().index(pd.Timestamp(test_date)) - 1
...
if "原版" in results and results["原版"]["total"] > 0:
    ...
    for vname in ["放宽C", "放宽D"]:
        if vname in results:
            r = results[vname]
            print(
                f"  {vname}: 信号{r['total'] / base['total']:.2f}x, 收益{r['avg_return'] / base['avg_return']:.2f}x, 胜率{r['win_rate'] / base['win_rate']:.2f}x"
            )
