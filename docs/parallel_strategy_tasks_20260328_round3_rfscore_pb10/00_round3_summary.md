...
- 一句话结论：**⚠️ 重大发现：PB10 候选仅0.3只/月，100%需补位，建议直接改用 PB20% 主策略（年化15.75%），绝对禁用 Score≥6 补位（年化-20.95%）**
- 是否进入正式版本：⚠️ **需要修正** - 原 PB10 设计需调整
- 关键发现：
  - **PB10 候选极其稀缺**: 平均 0.3只/月，100% 月份需补位
  - **Score≥6 是灾难**: 年化 -20.95%，最大回撤 -48.95%
  - **PB20 是唯一可行**: 年化 15.75%，胜率 65.4%
  - **修正建议**: 改用 PB20% 为主策略 (g.primary_pb_group = 2)
- Notebook 验证：
  - 运行时间：2026-04-01
  - 结果文件：`output/joinquant-notebook-result-策略测试-1775029364992.json`
...
- 一句话结论：**当前市场宽度23%处于底部区域，建议减仓至10只（50%仓位），保留50%现金，双月调仓**
- 是否进入正式版本：✅ **是，建议采用"无空仓线+双月调仓"配置**
- 关键发现：
  - **实测市场宽度**: 23.00%（底部区域）
  - **趋势状态**: 向下（弱势）
  - **当前建议**: 10只，PB10%，50%仓位，双月调仓
  - **核心规则**: 宽度<25%减仓至10只，不设空仓线（最低5只）
  - **现金储备**: 建议保留30-50%现金应对底部波动
...
- **历史基准库建设**：
  - 结果文件：`result_06_historical_baseline_database.md`
  - 数据文件：`data/rfscore_baseline_v1.0.json`
  - 核心发现：
    - 候选数量基准：月均1.5只，P50=1只，33.3%月份无候选
    - 估值基准：PB均值1.0，PE均值15.0
    - 盈利基准：ROA均值2.5%，OCFOA均值0.05
    - 分散度基准：行业数2个，HHI指数0.40，周期占比60%
    - 因子有效性基准：信号强度0.68
  - 当前vs基准对比：
    - 候选数量4只 vs 基准1.5只（2.67x 🔴严重偏高）
    - ROA均值1.26% vs 基准2.5%（0.50x 🟡偏低）
    - PB/PE估值正常（✓符合基准）
...
- **当前第三轮状态：✅ 已完成封版**（01✅ 02✅ 03✅ 04✅ 05✅ 06✅ 07✅ 08⏸️ 09✅ 10✅，完成率90%）
- **封版日期**：2026-03-31
- **封版文档**：`round3_release_v1.0.md` + `round3_final_summary_2026-03-31.md`
- 当前最接近正式版的 RFScore 方案：**PB10% 主线 + PB20% 备用池（`rfscore7_pb10_final.py`）**
- 当前推荐的防守搭配：**RFScore PB10 + 红利小盘 60/40 组合，建议替代30-40% ETF观察仓**
- 当前最大未解决问题：**PB10候选稀疏问题**（月均1.5只，33.3%月份无候选，已通过PB20备用池解决）
- **历史基准库**：v1.0已建立，支持偏离度判断（`data/rfscore_baseline_v1.0.json`）

### 关键结论

1. **口径已对齐**: 所有脚本统一使用 PB10%（primary_pb_group = 1）
2. **⚠️ PB10候选稀疏已验证**: 真实数据验证显示平均仅0.3只/月，100%需补位，建议改用PB20%
3. **异常已识别**: PE>100、ROE<2% 等异常值需要过滤规则
4. **建议过滤条件**: PE < 100 + ROA > 0.5% 硬过滤
5. **备用池已验证**: PB10+PB20两层架构实测等同于PB20严格（因PB10候选太少）
6. **绝对禁用**: Score≥6补位（年化-20.95%，最大回撤-48.95%）
7. **过滤器终审完成**:
...
9. **调仓机制优化**: 推荐混合调仓（月度+季报+风控），年预算15-17次
   - 最佳时机：周二至周四上午10:00-10:30
