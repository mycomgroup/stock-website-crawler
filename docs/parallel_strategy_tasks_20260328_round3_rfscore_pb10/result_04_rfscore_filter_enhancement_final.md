...
由于实际回测需要在JoinQuant环境中运行，建议在以下场景做完整验证：

### 7.1 必做回测场景

```python
# 使用 skills/joinquant_nookbook/run-rfscore-filter-final.js 执行
scenarios = [
    "rfscore_pb10_baseline",
    "rfscore_pb10_industry_cap_only",
    "rfscore_pb10_turnover_only", 
    "rfscore_pb10_cgo_only",
    "rfscore_pb10_turnover_cgo",  # Combined
    "rfscore_pb10_all_filters"
]
```
...
- **收益类**: 年化收益、累计收益、月胜率
- **风险类**: 最大回撤、夏普比率、Calmar比率
- **执行类**: 平均候选股数、调仓成功率、行业集中度

### 7.3 分阶段验证

1. **阶段1**: 2022-2023年（震荡+下跌）
2. **阶段2**: 2024-2025年（反弹+震荡）
3. **全周期**: 2022-2025年（完整周期）
...
| 过滤器 | 结论 | 理由 |
|--------|------|------|
| **Industry Cap** | ✅ **正式进入** | 基础风控，无争议 |
| **Turnover Filter** | ✅ **正式进入** | 负向显著，可执行性强 |
| **CGO Filter** | ✅ **正式进入（可选）** | 有边际价值，但需根据市场状态调整 |
| **Combined Filter** | ❌ **淘汰** | 过度复杂，稀疏性风险 |
...
1. **立即执行**: 在正式版代码中接入 Industry Cap + Turnover Filter
2. **待验证**: 跑完JoinQuant回测后确认CGO Filter参数
3. **监控**: 上线后监控候选股数量和过滤器触发频率
