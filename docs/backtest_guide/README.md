...
| 条件 | 推荐平台 | Skill 目录 |
|------|---------|-----------|
| **初步验证策略** | JoinQuant Notebook | `skills/joinquant_notebook` |
| **效果较好，需要精确回测** | JoinQuant 策略编辑器 | `skills/joinquant_strategy`（待完善） |
| **因子不多，代码改动简单** | RiceQuant Notebook | `skills/ricequant_strategy` |
| **因子多，代码改动复杂** | JoinQuant | `skills/joinquant_notebook` |

### RiceQuant vs JoinQuant 对比

| 特性 | JoinQuant | RiceQuant |
|------|-----------|-----------|
| Session 管理 | 需手动抓取 | **自动管理** |
| 因子库 | jqfactor（丰富） | fundamentals（有限） |
| 特殊因子 | 技术分析/质量/价值/动量/波动率 | 需手动计算 |
| API 稳定性 | 稳定 | 稳定 |
| 适合场景 | 复杂因子策略 | 简单因子策略 |
