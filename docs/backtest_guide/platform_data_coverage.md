...
## 十三、回测/选股最容易漏掉但很关键的数据

> 这些数据的关键不只是“有没有”，更重要的是是否支持**历史回溯**、是否按**当时口径**提供、是否会引入未来函数。

| 数据项 | 为什么关键 | 常见用途 | 平台提醒 / 应对 |
|------|-----------|---------|----------------|
| **ST / *ST / 退市整理 标记** | 不过滤会显著拉低可交易性，且涨跌停制度不同 | 剔除高风险股票、限制池过滤 | 优先用支持历史状态回溯的平台；若平台仅给当前状态，需避免未来函数 |
| **上市日期 / 退市日期 / 次新股过滤** | 次新股波动和流动性特征与老股差异很大；退市股会影响样本完整性 | 过滤上市未满 N 日股票、构建可投资股票池 | 需要历史可回溯的证券主表，不能只看当前存续股票列表 |
| **交易日历 / 停复牌原因** | 直接影响是否能买卖、能否成交，以及回测成交真实性 | 下单前过滤、停牌风控、持仓冻结处理 | 仅有停牌标记还不够，最好同时有停牌原因、停复牌日期 |
| **除权除息明细 / 复权因子** | 价格、收益率、涨跌幅、分红再投资都会受影响 | 前复权/后复权价格、总收益计算、股息策略 | 必须明确各平台默认复权口径；跨平台比对时要统一复权方式 |
| **指数成分股历史变更** | 用当前成分股回测历史会产生严重幸存者偏差 | 指数增强、基准池构建、成分股轮动 | 必须拿历史成分，不可用当前成分倒推历史 |
| **行业分类历史变更** | 公司行业归属会变化，当前行业标签不能代表历史 | 行业中性、行业轮动、行业暴露分析 | 若平台只支持当前行业，不适合严格历史行业回测 |
| **业绩预告 / 业绩快报 / 回购 / 减持** | 这些都是强事件驱动信号，发布时间点比字段本身更重要 | 事件驱动、财报预期差、股东行为因子 | 必须使用公告日/披露日，不要误用报告期或当前汇总值 |
| **基金持仓 / ETF 成分与申赎清单** | ETF、基金重仓、被动资金流影响很大 | ETF 套利、ETF 轮动、机构拥挤度分析 | ETF 策略不能只看净值或K线，成分和申赎清单常常才是核心数据 |
| **可转债** | 正股与转债联动、赎回条款、转股价调整会影响策略结果 | 转债轮动、低溢价策略、股债联动 | 需要债券行情 + 条款事件 + 转股价/赎回公告的联合数据 |
| **融券券源 / 借券费率** | 能不能融到券、融券成本多少，决定做空策略是否可实现 | 融券策略、对冲策略、市场中性 | 大多数免费平台覆盖不足，若无券源/费率，只能做理论回测 |
| **成交额 / 换手率 / 自由流通市值 的历史一致性** | 小市值、流动性、换手选股对口径极其敏感 | 小市值策略、流动性过滤、容量评估 | 必须确认是否为当日口径、是否复权一致、自由流通股本是否历史回补 |

### 13.1 这些数据最容易导致的回测偏差

| 风险 | 典型错误 | 后果 |
|------|---------|------|
| **未来函数** | 用当前 ST 状态、当前行业、当前指数成分回测历史 | 收益虚高，结论失真 |
| **幸存者偏差** | 只在当前股票池中筛选历史样本，忽略退市股票 | 历史收益被高估 |
| **成交可实现性偏差** | 忽略停牌、退市整理、融券券源、ETF 申赎约束 | 策略纸面可做，实盘不可做 |
| **口径不一致** | 市值、换手率、复权方式、公告日期口径混用 | 平台间结果无法对齐 |

### 13.2 实务建议

1. **股票池类策略**：先确认 `ST`、上市天数、停牌、退市整理、历史行业、历史成分股是否齐全。
2. **事件驱动策略**：优先核对“**公告日 / 可见日**”而不是报告期。
3. **ETF / 转债 / 融券策略**：不要只看行情，必须确认成分、条款、券源等配套数据。
4. **跨平台对比**：统一复权方式、市值单位、自由流通市值口径，再比较结果。
5. **免费数据补洞**：缺少事件类和结构化补充数据时，可优先考虑 AKShare；若要求严格历史口径，优先使用支持历史回溯的平台。
