...
**2. 股票池筛选**
- ✅ 根据策略逻辑筛选股票池
- ❌ 不要简单截断 stocks[:500]（会破坏策略逻辑）
- ✅ 排除ST、停牌、科创板等
- ✅ 股票数量 < 1000

**3. 因子数量**
- ✅ 核心因子 < 10个
- ✅ 批量获取：get_factor(stocks, factors, ...)
- ❌ 不要循环获取
...
- 批量获取因子
- 不在循环中获取数据
- 历史数据长度合理（count 20-60）

详细检查清单：PRE_RUN_CHECKLIST.md
```
...
1. **提交回测前**：先阅读"回测提交前检查提示词" ⭐
2. **首次运行策略**：使用"标准提示词"
3. **快速运行已知策略**：使用"精简提示词"
4. **运行特定平台策略**：使用对应平台提示词
5. **精确回测（推荐）**：使用"RiceQuant 策略编辑器提示词"
6. **快速验证逻辑**：使用"RiceQuant Notebook 提示词"
7. **转换策略格式**：使用"策略转换提示词"
8. **跨平台迁移**：使用"迁移提示词"
