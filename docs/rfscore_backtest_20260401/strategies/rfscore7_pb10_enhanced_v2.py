# RFScore7 + PB10% 增强策略 (RiceQuant 版)
# 增强特性: 四档仓位 (RFScore=2 满仓, RFScore=1 七成仓)
...
def rebalance(context, bar_dict):
    ...
    # 四档仓位: 根据 RFScore 分配仓位
    portfolio = context.portfolio
    total_value = portfolio.total_value

    # 从 factor_table 获取 RFScore
    stock_scores = {}
    for stock in target_stocks:
        if stock in factor_table.index:
            stock_scores[stock] = factor_table.loc[stock, "RFScore"]
        else:
            stock_scores[stock] = 1

    # 计算加权目标市值: RFScore=2 满仓, RFScore=1 七成仓
    total_weight = sum(1.0 if score == 2 else 0.7 for score in stock_scores.values())

    if total_weight == 0:
        total_weight = len(target_stocks)

    base_value = total_value / total_weight
    ...
    for stock in target_stocks:
        ...
        # 四档仓位
        score = stock_scores.get(stock, 1)
        position_ratio = 1.0 if score == 2 else 0.7
        target_value = base_value * position_ratio

        current_position = portfolio.positions.get(stock)
        current_value = current_position.market_value if current_position else 0

        # 调整到目标市值
        if abs(current_value - target_value) > target_value * 0.05:
