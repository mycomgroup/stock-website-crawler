...
三档定义：
- 满仓档：情绪热（涨停家数>50，连板数>5）
- 半仓档：情绪中（涨停家数30-50，连板数3-5）
- 空仓档：情绪冷（涨停家数<30，连板数<3）
"""
...
def get_position_type_detail(zt_count, max_lianban, position_ratio):
    """获取更详细的仓位类型标签"""
    if position_ratio == 1.0:
        return f"满仓(涨停{zt_count},连板{max_lianban})"
    elif position_ratio == 0.5:
        return f"半仓(涨停{zt_count},连板{max_lianban})"
    else:
        return f"空仓(涨停{zt_count},连板{max_lianban})"
...
for i, date in enumerate(trade_days):
    ...
    position_detail = get_position_type_detail(zt_count, max_lianban, position_ratio)
...
if all_trades:
    ...
    print(f"\n【仓位分布】")
    position_counts = df_positions["position_type"].value_counts()
    for pos_type, count in position_counts.items():
        print(f"{pos_type}: {count}天 ({count / len(df_positions) * 100:.1f}%)")

    print(f"\n【各档位交易统计】")
    for pos_type in ["满仓", "半仓"]:
        pos_trades = df_trades[df_trades["position_type"] == pos_type]
        if len(pos_trades) > 0:
            print(f"\n{pos_type}档位:")
            print(f"  交易次数: {len(pos_trades)}")
            print(
                f"  胜率: {(pos_trades['pnl_pct'] > 0).sum() / len(pos_trades) * 100:.2f}%"
            )
            print(f"  平均收益: {pos_trades['pnl_pct'].mean():.2f}%")
            print(f"  累计收益: {pos_trades['pnl_pct'].sum():.2f}%")
    ...
    print(f"  满仓天数: {len(df_positions[df_positions['position_type'] == '满仓'])}")
    print(f"  半仓天数: {len(df_positions[df_positions['position_type'] == '半仓'])}")
    print(f"  空仓天数: {len(df_positions[df_positions['position_type'] == '空仓'])}")

    df_trades.to_csv("/tmp/level_b_trades.csv", index=False)
    df_positions.to_csv("/tmp/level_b_positions.csv", index=False)
    print(f"\n详细数据已保存到: /tmp/level_b_trades.csv, /tmp/level_b_positions.csv")
