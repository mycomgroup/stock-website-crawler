...
def initialize(context):
    ...
    # 规则D: 无空仓线配置
    # 正常20只，宽度<25%减仓至10只，永不空仓
    g.base_hold_num = 20
    g.reduced_hold_num = 10
    g.breadth_reduce = 0.25
    g.breadth_stop = 0.00  # 永不空仓，最低保持reduced_hold_num
...
def rebalance(context):
    ...
    # 规则D: 无空仓线 - 永不空仓，最低保持10只
    # 注意：breadth_stop设为0表示不触发空仓
    if market_state["breadth"] < g.breadth_reduce and not market_state["trend_on"]:
        target_hold_num = g.reduced_hold_num  # 10只
    ...
    log.info(
        f"[规则D-无空仓] 宽度={market_state['breadth']:.2%} 趋势={'向上' if market_state['trend_on'] else '向下/震荡'} 持仓={target_hold_num}"
    )
    ...
    if target_stocks:
        target_value = context.portfolio.total_value / max(len(target_stocks), 1)
        current_data = get_current_data()
