...
def initialize(context):
    ...
    # 规则C: 激进型配置
    # 正常20只，宽度<20%减仓至15只，<10%空仓
    g.base_hold_num = 20
    g.reduced_hold_num = 15
    g.breadth_reduce = 0.20
    g.breadth_stop = 0.10
...
def rebalance(context):
    ...
    log.info(
        f"[规则C-激进] 宽度={market_state['breadth']:.2%} 趋势={'向上' if market_state['trend_on'] else '向下/震荡'} 持仓={target_hold_num}"
    )
