...
type: "website"
title: "get_price移动窗口"
url: "https://www.joinquant.com/help/api/doc?name=JQDatadoc&id=9874"
description: "描述"
source: ""
tags: []
crawl_time: "2026-03-27T07:15:01.826Z"
metadata:
  parser: "JoinquantHelpParser"
  actualUrl: "https://www.joinquant.com/help/api/doc?name=JQDatadoc&id=9874"
  headings:
    - {"level":3,"text":"get_price移动窗口","id":""}
    - {"level":5,"text":"示例：","id":""}
  paragraphs:
    - "描述"
    - "注意"
  ...
  mainContent:
    ...
    - {"type":"table","headers":["字段名称","中文名称","注释（特殊说明）"],"rows":[["open","时间段开始时价格",""],["close","时间段结束时价格",""],["low","时间段中的最低价",""],["high","时间段中的最高价",""],["volume","时间段中的成交的标的数量",""],["money","时间段中的成交的金额",""],["factor","复权因子","当fq参数分别指定以下值时，返回对应的复权因子：前复权(默认)，返回前复权因子;None:不复权,返回不复权因子;后复权，返回后复权因子;注（复权因子与复权数据的关系）：前(后)复权数据=价格×前(后)复权因子;前(后)复权后的成交量=成交量 / 前(后)复权因子;成交额不处理"],["high_limit","指定交易日的当日涨停价",""],["low_limit","指定交易日的当日跌停价",""],["avg","时间段中的平均价","成交额除以成交量；"],["pre_close","前一个单位时间结束时的价格,按天则是前一天的收盘价","前一个单位时间结束时的价格。注：按天来获取的话，股票、指数、基金、可转债的是前一天的收盘价。（新股上市为IPO发行价，股票出现分红，则pre close是指昨天收盘价带分红拆股调整后的价格）;期货/期权是前一天的结算价；"],["paused","bool值,股票是否停牌;","当paused=1，即停牌时 open/close/low/high/pre_close 都等于停牌前的收盘价, volume=money=0"],["open_interest","持仓量","期货/期权品种的持仓量"]]}
    - {"type":"heading","level":5,"content":"示例："}
    - {"type":"codeblock","language":"python","content":"#查询平安银行行情数据,round=False\ndf =get_price('000001.XSHE', start_date= '2015-01-28 09:00:00',end_date='2015-01-30 14:00:00',fq='post', frequency='minute', fields=['open','close','low','high','volume','money','factor',\n        'high_limit','low_limit','avg','pre_close','paused','factor'],round=False) \nprint(df[:4])\n\n                           open       close         low        high  \\\n2015-01-28 09:31:00  1073.13577  1069.26722  1068.49351  1073.13577   \n2015-01-28 09:32:00  1069.26722  1070.81464  1069.26722  1071.58835   \n2015-01-28 09:33:00  1070.81464  1072.36206  1070.04093  1072.36206   \n2015-01-28 09:34:00  1071.58835  1073.13577  1071.58835  1073.13577   \n\n                           volume       money  factor  high_limit  low_limit  \\\n2015-01-28 09:31:00  17496.051492  18749610.0  77.371  1190.73969  974.10089   \n2015-01-28 09:32:00   6858.732600   7337623.0  77.371  1190.73969  974.10089   \n2015-01-28 09:33:00   7126.442724   7633262.0  77.371  1190.73969  974.10089   \n2015-01-28 09:34:00   3460.521384   3711657.0  77.371  1190.73969  974.10089   \n\n                            avg   pre_close  paused  factor  \n2015-01-28 09:31:00  1071.58835  1082.42029     0.0  77.371  \n2015-01-28 09:32:00  1070.04093  1069.26722     0.0  77.371  \n2015-01-28 09:33:00  1070.81464  1070.81464     0.0  77.371  \n2015-01-28 09:34:00  1072.36206  1072.36206     0.0  77.371"}
    - {"type":"list","listType":"ul","items":["返回panel格式"]}
    - {"type":"codeblock","language":"python","content":"# panel=True时且pandas<0.25时返回[pandas.Panel]对象 , panel= False 或 pandas>=0.25时返回[pandas.DataFrame]对象\n\npanel=get_price(['000300.XSHG', '000001.XSHE'] ,count=2, panel=True)\n#返回[pandas.DataFrame]对象,行索引是[datetime.datetime]对象, 列索引是股票代号\ndf=panel['money']\nprint(df)\n\n             000300.XSHG  000001.XSHE\n2015-12-30  1.557441e+11  641277248.0\n2015-12-31  1.450437e+11  591643584.0"}
  suggestedFilename: "doc_JQDatadoc_9874_overview_get_price移动窗口"
  pageKind: "doc"
  pageName: "JQDatadoc"
  pageId: "9874"
  sectionHash: ""
...
https://www.joinquant.com/help/api/doc?name=JQDatadoc&id=9874
...
- round True：round到固定的小数位数，股票2位基金3位； 为False不进行round , 返回原始数值 ；
