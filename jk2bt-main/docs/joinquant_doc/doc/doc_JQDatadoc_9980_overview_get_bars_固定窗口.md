...
type: "website"
title: "get_bars 固定窗口"
url: "https://www.joinquant.com/help/api/doc?name=JQDatadoc&id=9980"
description: "描述"
source: ""
tags: []
crawl_time: "2026-03-27T07:23:57.820Z"
metadata:
  parser: "JoinquantHelpParser"
  actualUrl: "https://www.joinquant.com/help/api/doc?name=JQDatadoc&id=9980"
  headings:
  ...
  lists:
    ...
    - {"type":"ul","items":["当天 09:00 ~ 15:00 的行情在 15:00 之后可以获取 ,当end_dt指定为当天尚未结束的交易时间时，会自动填充为上一个交易日结束的时间","自2020年1月1日起，交易所调整商品期货【成交量和持仓量和成交额】的统计、发布和报送口径由双边计算统一为单边计算。聚宽的数据处理规则与交易所保持一致，如考虑前后统计口径保持一尺，可将相关字段的数值除以二用于获得大致结果。"]}
    ...
    - {"type":"ul","items":["获取一只期货合约"]}
    - {"type":"ul","items":["获取多只期货合约"]}
  tables:
    ...
    - {"caption":"","headers":["字段名称","中文名称","注释（特殊说明）"],"rows":[["date","日期",""],["open","时间段开始时价格",""],["close","时间段结束时价格",""],["low","时间段中的最低价",""],["high","时间段中的最高价",""],["volume","时间段中的成交的标的数量",""],["money","时间段中的成交的金额",""],["factor","复权因子","复权选项仅针对股票和基金品种有效，对于期货而言，复权因子被固定设置为1"],["open_interest","持仓量","期货/期权品种；反映当前时刻的持有数量"],["paused","是否停牌","0 正常；1 停牌 (限unit为1d时) (jqdatasdk1.9.5新增)"],["high_limit","当天涨停价","限unit为1d时 (jqdatasdk1.9.5新增)"],["low_limit","当天跌停价","限unit为1d时 (jqdatasdk1.9.5新增)"],["avg","当天均价","限unit为1d时 (jqdatasdk1.9.5新增)"],["pre_close","前收价","限unit为1d时 (jqdatasdk1.9.5新增)前天的收盘价。注：按天来获取的话，股票、指数、基金、可转债的是前一天的收盘价。（新股上市为IPO发行价，股票出现分红，则pre close是指昨天收盘价带分红拆股调整后的价格）;期货/期权是前一天的结算价；"]]}
  codeBlocks:
  ...
  mainContent:
    ...
    - {"type":"list","listType":"ul","items":["当天 09:00 ~ 15:00 的行情在 15:00 之后可以获取 ,当end_dt指定为当天尚未结束的交易时间时，会自动填充为上一个交易日结束的时间","自2020年1月1日起，交易所调整商品期货【成交量和持仓量和成交额】的统计、发布和报送口径由双边计算统一为单边计算。聚宽的数据处理规则与交易所保持一致，如考虑前后统计口径保持一尺，可将相关字段的数值除以二用于获得大致结果。"]}
    ...
    - {"type":"table","headers":["字段名称","中文名称","注释（特殊说明）"],"rows":[["date","日期",""],["open","时间段开始时价格",""],["close","时间段结束时价格",""],["low","时间段中的最低价",""],["high","时间段中的最高价",""],["volume","时间段中的成交的标的数量",""],["money","时间段中的成交的金额",""],["factor","复权因子","复权选项仅针对股票和基金品种有效，对于期货而言，复权因子被固定设置为1"],["open_interest","持仓量","期货/期权品种；反映当前时刻的持有数量"],["paused","是否停牌","0 正常；1 停牌 (限unit为1d时) (jqdatasdk1.9.5新增)"],["high_limit","当天涨停价","限unit为1d时 (jqdatasdk1.9.5新增)"],["low_limit","当天跌停价","限unit为1d时 (jqdatasdk1.9.5新增)"],["avg","当天均价","限unit为1d时 (jqdatasdk1.9.5新增)"],["pre_close","前收价","限unit为1d时 (jqdatasdk1.9.5新增)前天的收盘价。注：按天来获取的话，股票、指数、基金、可转债的是前一天的收盘价。（新股上市为IPO发行价，股票出现分红，则pre close是指昨天收盘价带分红拆股调整后的价格）;期货/期权是前一天的结算价；"]]}
    - {"type":"heading","level":5,"content":"示例："}
    - {"type":"list","listType":"ul","items":["获取一只期货合约"]}
    - {"type":"codeblock","language":"python","content":"#获取A0603期货合约在“2018-12-05”前5个时间单位的数据\n\ndf = get_bars('A0603.XDCE', 5, unit='120m',fields=['open','close','low','high','volume','money','open_interest'],include_now=False,end_dt='2006-03-15')\nprint(df)\n                 date    open   close     low    high  volume      money  \\\n0 2006-03-13 11:30:00  2603.0  2603.0  2603.0  2603.0     0.0        0.0   \n1 2006-03-13 15:00:00  2603.0  2603.0  2603.0  2603.0     0.0        0.0   \n2 2006-03-14 11:00:00  2603.0  2581.0  2581.0  2603.0   210.0  5422000.0   \n3 2006-03-14 11:30:00  2581.0  2610.0  2581.0  2610.0    48.0  1252800.0   \n4 2006-03-14 15:00:00  2610.0  2595.0  2581.0  2610.0    30.0   774580.0   \n\n   open_interest  \n0         1526.0  \n1         1526.0  \n2         1736.0  \n3         1784.0  \n4         1812.0"}
    - {"type":"list","listType":"ul","items":["获取多只期货合约"]}
    - {"type":"codeblock","language":"python","content":"df = get_bars(['A2109.XDCE', 'A2111.XDCE'], 5, unit='120m',fields=['open','close','low','high','volume','money','open_interest'],include_now=False,end_dt='2020-12-19')\nprint(df)\n\n\n                           date    open   close     low    high  volume  \\\nA2109.XDCE 0 2020-12-17 23:00:00  5227.0  5250.0  5216.0  5250.0   158.0   \n           1 2020-12-18 11:00:00  5255.0  5280.0  5252.0  5283.0   305.0   \n           2 2020-12-18 11:30:00  5280.0  5275.0  5275.0  5281.0     9.0   \n           3 2020-12-18 15:00:00  5275.0  5267.0  5255.0  5280.0    82.0   \n           4 2020-12-18 23:00:00  5259.0  5268.0  5255.0  5281.0   228.0   \nA2111.XDCE 0 2020-12-17 23:00:00  5145.0  5156.0  5141.0  5156.0     2.0   \n           1 2020-12-18 11:00:00  5159.0  5148.0  5148.0  5175.0     9.0   \n           2 2020-12-18 11:30:00  5148.0  5176.0  5148.0  5176.0     1.0   \n           3 2020-12-18 15:00:00  5150.0  5131.0  5131.0  5150.0    16.0   \n           4 2020-12-18 23:00:00  5151.0  5119.0  5112.0  5151.0    10.0   \n\n                   money  open_interest  \nA2109.XDCE 0   8273760.0         2544.0  \n           1  16074820.0         2521.0  \n           2    475060.0         2517.0  \n           3   4316740.0         2506.0  \n           4  12004590.0         2405.0  \nA2111.XDCE 0    102970.0           78.0  \n           1    464630.0           82.0  \n           2     51760.0           82.0  \n           3    823170.0           95.0  \n           4    511850.0          101.0"}
  suggestedFilename: "doc_JQDatadoc_9980_overview_get_bars_固定窗口"
  pageKind: "doc"
  pageName: "JQDatadoc"
  pageId: "9980"
  sectionHash: ""
...
https://www.joinquant.com/help/api/doc?name=JQDatadoc&id=9980
