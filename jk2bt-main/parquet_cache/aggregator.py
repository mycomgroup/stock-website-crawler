...
from .schema_validator import deduplicate_by_key
from .table_registry import CacheTable, DEFAULT_REGISTRY, TableRegistry
...
class Aggregator:
    def __init__(
        ...
        self.writer = AtomicWriter(self.base_dir)
    ...
    def aggregate_table(
        ...
        if not table_def.aggregation_enabled:
            return 0
        ...
        try:
            ...
            for partition_value in pending:
                ...
                effective_threshold = (
                    ...
                    else table_def.compaction_threshold
                )

                if file_count > effective_threshold:
                    success = self._aggregate_partition(table_def, partition_value)
                    if success:
                        aggregated_count += 1
    ...
    def aggregate_all(
        ...
        for name, table_def in tables.items():
            if not table_def.aggregation_enabled:
                continue
            if priority is not None and table_def.priority != priority:
                continue
    ...
    def cleanup(
        ...
        for name, table_def in tables.items():
            if not table_def.aggregation_enabled:
                continue
            if table_def.partition_by is None:
                continue
    ...
    def needs_aggregation(
        ...
        for partition_value in partitions:
            ...
            if len(files) > table_def.compaction_threshold:
    ...
    def _aggregate_partition(
        ...
        try:
            data = self._read_partition_raw(table_def, partition_value)
            if data is None or data.empty:
                return False

            data = deduplicate_by_key(data, table_def.primary_key)

            self._write_aggregated(table_def, partition_value, data)
    ...
    def _write_aggregated(
        ...
        try:
            ...
            pq.write_table(
                ...
                row_group_size=self.writer.row_group_size,
            )
            import os
