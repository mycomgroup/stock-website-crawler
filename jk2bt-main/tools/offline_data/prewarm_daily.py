...
def run_prewarm_daily(
    ...
    if etfs:
        ...
        etf_result = prewarm_etf_daily(
            etfs, start_date, end_date, skip_existing=skip_existing, force_update=force
        )
...
def main():
    ...
    print("=" * 60)
    print(
        f"股票: 成功={summary.get('stock_daily', {}).get('success', 0)}, 跳过={summary.get('stock_daily', {}).get('skipped', 0)}"
    )
    print(
        f"ETF: 成功={summary.get('etf_daily', {}).get('success', 0)}, 跳过={summary.get('etf_daily', {}).get('skipped', 0)}"
    )
    print(
        f"指数: 成功={summary.get('index_daily', {}).get('success', 0)}, 跳过={summary.get('index_daily', {}).get('skipped', 0)}"
    )
