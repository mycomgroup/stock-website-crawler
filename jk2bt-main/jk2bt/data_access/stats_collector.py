"""
stats_collector.py
轻量级统计收集系统 - 用于监控数据请求和缓存性能。

提供:
- RequestStats: 请求统计（成功率、延迟、错误分布）
- CacheStats: 缓存统计（命中率）
- StatsCollector: 单例收集器，聚合所有统计信息

使用方式:
    from jk2bt.data_access.stats_collector import get_stats_collector

    stats = get_stats_collector()
    stats.record_request("akshare", 150.5, success=True, endpoint="stock_zh_a_daily")
    stats.record_cache_hit("market_cache")
...
class StatsCollector:
    ...
    def get_summary_text(self) -> str:
        ...
        lines.append("=" * 50)
        lines.append("  JK2BT Stats Summary")
        lines.append("=" * 50)
