...
def check_dependencies(
    ...
    for name, dep_info in DEPENDENCY_REGISTRY.items():
        ...
        if verbose:
            ...
            if available:
                logger.info(f"[{level_str}] {name}: {status} (v{version_or_error})")
            else:
                logger.warning(f"[{level_str}] {name}: {status}")
                if dep_info.alternative:
                    logger.info(f"    -> 替代方案: {dep_info.alternative}")
                logger.info(f"    -> 安装命令: {dep_info.install_cmd}")
...
def check_required_dependencies() -> bool:
    ...
    if missing:
        logger.error("=" * 60)
        logger.error("缺少核心依赖，策略无法运行:")
        for name in missing:
            dep = DEPENDENCY_REGISTRY[name]
            logger.error(f"  - {name}: {dep.install_cmd}")
        logger.error("=" * 60)
        return False
