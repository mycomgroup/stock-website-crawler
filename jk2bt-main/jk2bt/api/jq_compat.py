    ...
    try:
        ...
        if robust:
            return RobustResult(
                ...
                data=result_data,
                reason=f"成功获取 {len(result_data)} 只成分股权重"
    ...
    if isinstance(result, dict) and len(result) > 1 and panel:
        ...
        for sym, df in result.items():
            if not df.empty:
                ...
                elif df.index.name == "datetime" or isinstance(
        ...
        # 确定要转换的字段
        fields_to_convert = (
            fields if fields else ["open", "high", "low", "close", "volume", "money"]
        )
...
def get_billboard_list_jq(
    ...
    委托给 api/billboard.py 的完整版实现。
    """
    from jk2bt.api.billboard import get_billboard_list as _billboard_impl

    # 转换参数：JQ 风格 → billboard.py 风格
    stock = stock_list
    start_date = None

    if end_date and not isinstance(end_date, str):
        end_date = str(end_date).replace("-", "")[:10]

    return _billboard_impl(
        stock=stock, start_date=start_date, end_date=end_date, count=count
    )
...
def winsorize_med(factor_data, scale=3, inclusive=True, inf2nan=True, axis=0):
    ...
    try:
        ...
        raise NotImplementedError(
            "winsorize_med 需要安装 jqfactor_analyzer 或 factors.preprocess 模块"
        )
...
def standardlize(factor_data, inf2nan=True, axis=0):
    ...
    try:
        ...
        raise NotImplementedError(
            "standardlize 需要安装 jqfactor_analyzer 或 factors.preprocess 模块"
        )
