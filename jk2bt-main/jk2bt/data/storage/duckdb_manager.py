...
class DuckDBManager:
    ...
    def get_stock_daily(
        ...
        if use_cache:
            cached = _global_cache.get(
                "stock_daily", jq_symbol, start, end, adjust=adjust
            )
        ...
        with self._get_connection(read_only=True) as conn:
            ...
            if not df.empty:
                ...
                if use_cache:
                    _global_cache.set(
                        "stock_daily", jq_symbol, start, end, df, adjust=adjust
                    )
            else:
    ...
    def get_etf_minute(
        ...
        if use_cache:
            cached = _global_cache.get(
                "etf_minute", jq_symbol, start, end, period=period
            )
            if cached is not None:
                return cached
        ...
        with self._get_connection(read_only=True) as conn:
            ...
            if use_cache and not df.empty:
                _global_cache.set(
                    "etf_minute", jq_symbol, start, end, df, period=period
                )
...
def get_manager(
    db_path: str = None, read_only: bool = False, use_cache: bool = True
) -> DuckDBManager:
    """
    获取管理器实例的统一入口（推荐使用）
