...
from jk2bt.utils.date_utils import parse_date as _parse_date, parse_num, parse_int
...
OPTION_TICK_SCHEMA = _OPTION_TICK_SCHEMA


class OptionDBManager(LazyInitSingleton):
    """期权 DuckDB 管理器（延迟初始化）"""
    ...
    def _do_init(self, db_path: str = None):
...
def get_option_list(
    ...
    try:
        from jk2bt.data.sources import get_adapter as _get_adapter_opt
...
def _parse_sse_option_row(row, date_str: str) -> Optional[dict]:
    ...
    try:
        ...
        return {
            ...
            "strike": parse_num(row.get("行权价", 0)),
            "expiry_date": _parse_date(row.get("到期日", None)),
            "option_type": option_type,
            "contract_unit": parse_int(row.get("合约单位", 0)),
            "exercise_type": "欧式",
            "listing_date": None,
            "close": None,
            "volume": None,
...
def _parse_szse_option_row(row, date_str: str) -> Optional[dict]:
    ...
    try:
        ...
        return {
            ...
            "exercise_type": "欧式",
            "listing_date": None,
            "close": parse_num(row.get("前结算价", 0)),
            "volume": parse_int(row.get("合约总持仓", 0)),
            "date": date_str,
...
def _parse_cffex_option_row(row, date_str: str, opt_type: str) -> Optional[dict]:
    ...
    try:
        ...
        return {
            ...
            "exercise_type": "欧式",
            "listing_date": None,
            "close": parse_num(row.get(f"{prefix}-最新价", 0)),
            "volume": parse_int(row.get(f"{prefix}-持仓量", 0)),
            "date": date_str,
        }
    except Exception:
        return None
    ...
    try:
        ...
        if df is not None and not df.empty:
            ...
            result = {
                ...
                "delta": parse_num(greeks_dict.get("Delta")),
                "gamma": parse_num(greeks_dict.get("Gamma")),
                "theta": parse_num(greeks_dict.get("Theta")),
                "vega": parse_num(greeks_dict.get("Vega")),
                "implied_vol": parse_num(greeks_dict.get("隐含波动率")),
                "strike": parse_num(greeks_dict.get("行权价")),
                "last_price": parse_num(greeks_dict.get("最新价")),
                "theoretical_value": parse_num(greeks_dict.get("理论价值")),
                "volume": parse_int(greeks_dict.get("成交量")),
                "date": date_str,
            }
...
def get_option_ticks(
    ...
    exchange : str, optional
        交易所: 'sse' (上交所), 'szse' (深交所), 'czce' (郑商所),
        'shfe' (上期所), 'dce' (大商所), 'cffex' (中金所)

    返回
    ----
    RobustResult，data 包含 tick 数据，字段:
    - datetime: 时间戳
    - option_code: 期权代码
    - price: 最新价
    - volume: 成交量
    ...
    try:
        ...
        if exchange is None:
            exchange = _infer_exchange_from_code(option_code)

        if exchange in ("sse", "szse"):
            df = _fetch_financial_option_ticks(
                option_code, start_date, end_date, exchange
            )
        elif exchange in ("czce", "shfe", "dce", "cffex"):
            df = _fetch_commodity_option_ticks(
                option_code, start_date, end_date, exchange
            )
        else:
            ...
            return RobustResult(
                success=False,
                data=pd.DataFrame(columns=_OPTION_TICK_SCHEMA),
                reason=f"不支持的交易所: {exchange}",
            )
        ...
        return RobustResult(
            success=False,
            data=pd.DataFrame(columns=_OPTION_TICK_SCHEMA),
            reason=f"未获取到期权 {option_code} 的tick数据",
        )

    except Exception as e:
        logger.warning(f"[get_option_ticks] 获取tick数据失败: {e}")
        return RobustResult(
            success=False,
            data=pd.DataFrame(columns=_OPTION_TICK_SCHEMA),
            reason=str(e),
...
def _infer_exchange_from_code(option_code: str) -> str:
    """根据期权代码推断交易所"""
    ...
    if code.startswith(("51", "56", "58")):
        ...
        return "szse"
    elif code.startswith(("C", "P", "SR", "CF", "TA", "MA", "FG", "SA", "AP", "OI")):
        return "czce"
    elif code.startswith(("cu", "al", "zn", "pb", "ni", "sn", "au", "ag", "rb", "hc")):
        return "shfe"
    elif code.startswith(("m", "y", "p", "i", "j", "jm", "pp", "l", "v", "eb", "pg")):
        return "dce"
    elif code.startswith(("IC", "IF", "IH", "IM", "T", "TF", "TS")):
        return "cffex"
    return "sse"


def _fetch_financial_option_ticks(
    option_code: str,
    start_date: Optional[str],
    end_date: Optional[str],
    exchange: str,
) -> Optional[pd.DataFrame]:
    """获取金融期权tick数据（SSE/SZSE ETF期权）"""
    try:
        import akshare as ak

        if exchange == "sse":
            try:
                df = ak.option_sse_tick_sina(symbol=option_code)
                if df is not None and not df.empty:
                    return _normalize_tick_df(df, option_code)
            except Exception as e:
                logger.debug(f"option_sse_tick_sina 失败: {e}")

            try:
                df = ak.option_finance_minute_sina(symbol=option_code)
                if df is not None and not df.empty:
                    return _normalize_minute_to_tick(df, option_code)
            except Exception as e:
                logger.debug(f"option_finance_minute_sina 失败: {e}")
            ...
            try:
                ...
                logger.debug(f"szse minute 失败: {e}")

        logger.warning(f"akshare 暂无 {exchange} 期权tick数据接口")
        return pd.DataFrame(columns=_OPTION_TICK_SCHEMA)

    except Exception as e:
        logger.warning(f"获取金融期权tick失败: {e}")
        return pd.DataFrame(columns=_OPTION_TICK_SCHEMA)


def _fetch_commodity_option_ticks(
    option_code: str,
    start_date: Optional[str],
    end_date: Optional[str],
    exchange: str,
) -> Optional[pd.DataFrame]:
    """获取商品期权tick数据（目前akshare不支持）"""
    logger.warning(f"akshare 暂无 {exchange} 商品期权tick数据接口")
    return pd.DataFrame(columns=_OPTION_TICK_SCHEMA)


def _normalize_tick_df(df: pd.DataFrame, option_code: str) -> pd.DataFrame:
    """标准化tick数据DataFrame"""
    result = pd.DataFrame(columns=_OPTION_TICK_SCHEMA)
    ...
    datetime_col = None
    for c in ["时间", "datetime", "trade_time", "timestamp"]:
        if c in df.columns:
            datetime_col = c
            break
    ...
    result["datetime"] = pd.to_datetime(df[datetime_col])
    result["option_code"] = option_code

    price_col = None
    for c in ["最新价", "price", "last_price", "close"]:
        if c in df.columns:
            price_col = c
            break
    if price_col:
    ...
    for i in range(1, 6):
        bp_col = None
        for c in [f"买{i}", f"bid_price{i}", f"bp{i}"]:
            if c in df.columns:
                bp_col = c
                break
        if bp_col:
            result[f"bid_price{i}"] = pd.to_numeric(df[bp_col], errors="coerce")

        ap_col = None
        for c in [f"卖{i}", f"ask_price{i}", f"ap{i}"]:
            if c in df.columns:
                ap_col = c
                break
        if ap_col:
            result[f"ask_price{i}"] = pd.to_numeric(df[ap_col], errors="coerce")

        bv_col = None
        for c in [f"买{i}量", f"bid_volume{i}", f"bv{i}"]:
            if c in df.columns:
                bv_col = c
                break
        if bv_col:
            result[f"bid_volume{i}"] = pd.to_numeric(df[bv_col], errors="coerce")

        av_col = None
        for c in [f"卖{i}量", f"ask_volume{i}", f"av{i}"]:
            if c in df.columns:
                av_col = c
                break
        if av_col:
            result[f"ask_volume{i}"] = pd.to_numeric(df[av_col], errors="coerce")

    return result


def _normalize_minute_to_tick(df: pd.DataFrame, option_code: str) -> pd.DataFrame:
    """将分钟数据转换为tick格式（降级方案）"""
    result = pd.DataFrame(columns=_OPTION_TICK_SCHEMA)
    ...
    datetime_col = None
    for c in ["时间", "datetime", "trade_time"]:
        if c in df.columns:
            datetime_col = c
            break
...
def get_option_preopen(
    ...
    date : str, optional
        日期 'YYYY-MM-DD'，默认今日
    exchange : str, optional
        交易所: 'sse', 'szse', 'cffex', 'all'

    返回
    ----
    RobustResult，data 包含盘前数据，字段:
    - date: 日期
    - option_code: 期权代码
    - option_name: 期权名称
    ...
    try:
        ...
        exchanges = (
            ["sse", "szse"] if exchange is None or exchange == "all" else [exchange]
        )

        for ex in exchanges:
            try:
                if ex == "sse":
                    df = ak.option_sse_daily_sina()
                    if df is not None and not df.empty:
                        for _, row in df.iterrows():
                            results.append(
                                {
                                    ...
                                    "option_code": str(row.get("合约编码", "")),
                                    "option_name": str(row.get("合约简称", "")),
                                    "underlying_code": _extract_underlying_from_sse(
                                        row
                                    ),
                                    "pre_settle": parse_num(row.get("前结算价")),
                                    "pre_close": parse_num(row.get("前收盘价")),
                                    "pre_position": parse_int(row.get("前持仓量")),
                                    "limit_up": parse_num(row.get("涨停价")),
                                    "limit_down": parse_num(row.get("跌停价")),
                                }
                            )
                elif ex == "szse":
                    df = ak.option_szse_daily_sina()
                    if df is not None and not df.empty:
                        for _, row in df.iterrows():
                            results.append(
                                {
                                    ...
                                    "option_code": str(row.get("合约编码", "")),
                                    "option_name": str(row.get("合约简称", "")),
                                    "underlying_code": _extract_underlying_from_szse(
                                        row
                                    ),
                                    "pre_settle": parse_num(row.get("前结算价")),
                                    "pre_close": parse_num(row.get("前收盘价")),
                ...
                logger.debug(f"获取 {ex} 盘前数据失败: {e}")

        if results:
            result_df = pd.DataFrame(results)
            for col in preopen_schema:
                if col not in result_df.columns:
                    result_df[col] = None
            return RobustResult(
                success=True, data=result_df[preopen_schema], source="network"
            )

        logger.warning("akshare 暂无期权盘前静态数据接口，返回空DataFrame")
        return RobustResult(
            success=False,
            data=pd.DataFrame(columns=preopen_schema),
            reason="akshare 暂无期权盘前静态数据接口",
        )

    except Exception as e:
        logger.warning(f"[get_option_preopen] 获取盘前数据失败: {e}")
        return RobustResult(
            success=False,
            data=pd.DataFrame(columns=preopen_schema),
            reason=str(e),
...
def _extract_underlying_from_sse(row) -> str:
    """从SSE期权数据行提取标的代码"""
    underlying_str = str(row.get("标的券名称及代码", ""))
    if "(" in underlying_str and ")" in underlying_str:
        return underlying_str.split("(")[-1].replace(")", "")
    return ""


def _extract_underlying_from_szse(row) -> str:
    """从SZSE期权数据行提取标的代码"""
    underlying_str = str(row.get("标的证券简称(代码)", ""))
    if "(" in underlying_str and ")" in underlying_str:
        return underlying_str.split("(")[-1].replace(")", "")
...
def get_option_adjustment(option_code: str) -> pd.DataFrame:
    ...
    NotImplementedError: 期权调整记录需要专业数据源。请替换为其他数据源。
    """
