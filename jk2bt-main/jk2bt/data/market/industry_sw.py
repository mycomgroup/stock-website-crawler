...
class IndustrySWDBManager(LazyInitSingleton):
    """申万行业 DuckDB 管理器（延迟初始化）"""

    def __init__(self):
        # __init__ 可能在每次调用 IndustrySWDBManager() 时被调用，
        # 但实际初始化只在 ensure_initialized 中执行一次
...
from jk2bt.utils.symbol import normalize_symbol as _normalize_stock_code
