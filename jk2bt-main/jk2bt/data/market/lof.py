...
def _normalize_lof(df: pd.DataFrame) -> pd.DataFrame:
    """LOF 数据列名标准化。"""
    df = df.copy()
    columns_map = {
        "日期": "datetime",
...
def get_lof_daily(
    ...
    def _fetch():
        ...
        for attempt in range(retry_count):
            try:
                logger.info(
                    f"{symbol}: 尝试获取 LOF 数据 (第 {attempt + 1}/{retry_count} 次)"
                )
                ...
                last_error = e
                logger.warning(
                    f"{symbol}: 第 {attempt + 1} 次尝试失败 - {str(e)[:100]}"
                )
        ...
        if last_error:
            raise ValueError(f"{symbol}: LOF 数据获取失败 - {str(last_error)[:100]}")
        raise ValueError(f"{symbol}: LOF 数据获取失败")

    def _insert(sym, df):
        try:
            db_write = get_writer_manager()
            db_write.insert_lof_daily(sym, df)
        except Exception:
            pass

    return _cached_daily_fetch(
        table_name="lof_daily",
        symbol=symbol,
        start=start,
        end=end,
        fetch_fn=_fetch,
        insert_fn=_insert,
        normalize_fn=_normalize_lof,
...
def get_lof_min(
    ...
    from jk2bt.utils.standardize import standardize_minute_ohlcv
