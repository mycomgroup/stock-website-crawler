...
class StrategyCorpusAuditor:
    ...
    def __init__(
        self, strategies_dir: Optional[Path] = None, output_dir: Optional[Path] = None
    ):
        """
        初始化审计器
    ...
    def _fallback_scan(
        self, file_path: Path, file_type: str, scan_timestamp: str
    ) -> AuditResult:
        """兜底扫描（当scanner不可用时）"""
        try:
    ...
    def compare_with_registry(self, results: List[AuditResult]) -> Dict[str, Any]:
        ...
        comparison["new_files"] = sorted(list(audit_paths - registry_paths))[
            :20
        ]  # 只显示前20个
        ...
        # 一致性检查
        if (
            comparison["matched"] == len(results)
            and not comparison["new_files"]
            and not comparison["removed_files"]
        ):
    ...
    def save_csv_report(
        self, results: List[AuditResult], filename: str = "audit_report.csv"
    ) -> Path:
        """保存CSV报告"""
        ...
        with open(output_path, "w", encoding="utf-8", newline="") as f:
            ...
            for result in results:
                row = {
                    ...
                    "missing_apis": ",".join(result.missing_apis)
    ...
    def print_summary_report(self, summary: AuditSummary, comparison: Dict) -> None:
        ...
        for status, count in sorted(
            summary.status_distribution.items(), key=lambda x: -x[1]
        ):
        ...
        if summary.missing_api_distribution:
            ...
            for api, count in sorted(
                summary.missing_api_distribution.items(), key=lambda x: -x[1]
            )[:10]:
        ...
        if comparison["status_changed"][:5]:
            ...
            for change in comparison["status_changed"][:5]:
                print(
                    f"    * {change['file']}: {change['registry_status']} -> {change['audit_status']}"
                )
...
def test_registry_consistency():
    ...
    if comparison["registry_exists"]:
        ...
        if status == "inconsistent":
            ...
            print(
                f"差异详情: 新增{details['new_count']}, 移除{details['removed_count']}, 变更{details['changed_count']}"
            )
    else:
...
def test_file_type_statistics():
    ...
    for result in ipynb_results:
        assert not result.is_executable, f"notebook不应标记为可执行: {result.file_name}"
