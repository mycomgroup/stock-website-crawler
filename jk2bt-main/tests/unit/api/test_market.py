...
class TestGetSymbolPrefix:
    ...
    def test_sh_prefix_for_6(self):
        from jk2bt.utils.symbol import get_symbol_prefix
...
class TestIsGemOrStar:
    ...
    def test_gem_code(self):
        from jk2bt.utils.symbol import is_gem_or_star
...
class TestCalculateLimitPrice:
    ...
    def test_mainboard_up(self):
        from jk2bt.utils.symbol import calculate_limit_price
...
@pytest.fixture
def mock_get_adaptershare_daily():
    """模拟 AkShare 日线数据"""
...
class TestGetPriceReturnStructure:
    ...
    def test_single_security_returns_dataframe(
        ...
        result = get_price(
            "600519.XSHG", start_date="2023-01-01", end_date="2023-01-10"
        )
    ...
    def test_mainboard_limit_ratio_10pct(self, mock_get_adapter):
        ...
        for i in range(1, len(result)):
            if result.iloc[i]["paused"] == 0 and pd.notna(result.iloc[i]["pre_close"]):
                ...
                assert (
                    abs(result.iloc[i]["high_limit"] - round(pre_close * 1.10, 2))
                    < 0.01
                )
                assert (
    ...
    def test_gem_limit_ratio_20pct(self, mock_get_adapter):
        ...
        for i in range(1, len(result)):
            if result.iloc[i]["paused"] == 0 and pd.notna(result.iloc[i]["pre_close"]):
                pre_close = result.iloc[i]["pre_close"]
                assert (
                    abs(result.iloc[i]["high_limit"] - round(pre_close * 1.20, 2))
                    < 0.01
                )
                assert (
                    abs(result.iloc[i]["low_limit"] - round(pre_close * 0.80, 2)) < 0.01
                )
...
class TestCodeFormatCompatibility:
    ...
    def test_jq_format(self, mock_get_adapter, mock_data):
        ...
        result = get_price(
            "600519.XSHG", start_date="2023-01-01", end_date="2023-01-05"
        )
        assert isinstance(result, pd.DataFrame)
        assert (
            mock_get_adapter.return_value.get_stock_hist.call_args[1]["symbol"]
            == "600519"
        )
    ...
    def test_sz_format(self, mock_get_adapter, mock_data):
        ...
        result = get_price(
            "000858.XSHE", start_date="2023-01-01", end_date="2023-01-05"
        )
        assert isinstance(result, pd.DataFrame)
...
class TestHistoryWithMock:
    ...
    def test_history_returns_dataframe(self, mock_get_adapter, mock_data):
        ...
        result = history(
            ...
            unit="1d",
            field="close",
            security_list=["600519.XSHG", "000858.XSHE"],
        )
        assert isinstance(result, pd.DataFrame)
    ...
    def test_history_df_false_returns_dict(self, mock_get_adapter, mock_data):
        ...
        result = history(
            count=10, unit="1d", field="close", security_list=["600519.XSHG"], df=False
        )
        assert isinstance(result, dict)
        assert "600519.XSHG" in result
...
class TestAttributeHistoryWithMock:
    ...
    def test_attribute_history_returns_dataframe(self, mock_get_adapter, mock_data):
        ...
        result = attribute_history(
            "600519.XSHG", count=10, unit="1d", fields=["open", "close", "high", "low"]
        )
        assert isinstance(result, pd.DataFrame)
    ...
    def test_attribute_history_df_false(self, mock_get_adapter, mock_data):
        ...
        result = attribute_history(
            "600519.XSHG", count=10, unit="1d", fields=["open", "close"], df=False
        )
        assert isinstance(result, dict)
...
class TestGetBarsWithMock:
    ...
    def test_get_bars_multiple_securities(self, mock_get_adapter, mock_data):
        ...
        result = get_bars(["600519.XSHG", "000858.XSHE"], count=10, unit="1d")
        assert isinstance(result, dict)
        assert "600519.XSHG" in result
        assert "000858.XSHE" in result
