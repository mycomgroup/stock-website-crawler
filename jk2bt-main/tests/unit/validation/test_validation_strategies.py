...
class TestValidationStrategies:
    ...
    def test_all_strategies_loaded(self):
        ...
        assert len(self.strategies) == 7, (
            f"预期7个验收策略，实际{len(self.strategies)}个"
        )
        ...
        for strategy in self.strategies:
            for field in required_fields:
                assert field in strategy, (
                    f"策略{strategy.get('id', 'unknown')}缺少字段{field}"
                )
    ...
    def test_data_availability(self):
        ...
        if missing_data_strategies:
            ...
            for item in missing_data_strategies:
                msg += (
                    f"  {item['id']} ({item['name']}): {', '.join(item['missing'])}\n"
                )
            # 软性检查：不硬性失败，仅输出警告
    ...
    def test_run_strategy(self, strategy_idx):
        ...
        if len(all_pool) == 0:
            pytest.fail(
                f"策略{strategy['id']}股票池/ETF池为空，请检查配置（需要fixed_stock_pool fallback）"
            )
        ...
        try:
            ...
            assert "runtime_errors" in result, (
                f"策略{strategy['id']}结果缺少runtime_errors字段"
            )
            runtime_errors = result.get("runtime_errors", [])
            assert len(runtime_errors) == 0, (
                f"策略{strategy['id']}有{len(runtime_errors)}个runtime_errors，不应验收通过"
            )

            # 验证结果合理性
            assert result["final_value"] > 0, f"策略{strategy['id']}最终资金<=0"
            assert abs(result["pnl_pct"]) < 200, (
                f"策略{strategy['id']}收益率异常（>{200}%）"
            )
    ...
    def test_p0_strategies_must_pass(self):
        """P0优先级策略必须全部通过"""
        p0_strategies = [
            s for s in self.strategies if s.get("validation_priority") == "P0"
        ]
        ...
        for strategy in p0_strategies:
            ...
            if not os.path.exists(full_path):
                failed.append({"id": strategy["id"], "reason": "文件不存在"})
                continue
            ...
            try:
                ...
                if result is None:
                    failed.append({"id": strategy["id"], "reason": "运行结果为None"})
                elif result["final_value"] <= 0:
                    failed.append(
                        {"id": strategy["id"], "reason": "运行结果异常：final_value<=0"}
                    )
                ...
                failed.append({"id": strategy["id"], "reason": str(e)})
